       ...
       ULONG RunLength)
{
    ...
    // RunBuffer is one file record long, so that - not the cluster size - is the
    // real capacity. Passing BytesPerCluster here overflowed RunBuffer whenever a
    // cluster is larger than a file record (e.g. 4K clusters, 1K records), so a
    // write to a sufficiently fragmented file corrupted the heap and never
    // finished.
