    ...
    IN PUCHAR pciBuffer)
{
    ...
    ConfigInfo->MaximumNumberOfLogicalUnits = 1;  // One namespace = one LUN (required for scsiport to create PDO and send INQUIRY)
    ...
    OUT PBOOLEAN Again)
{
    ...
    if (HwContext) {
        ...
        ScsiDebugPrint(0, "nvme2k: HwFindAdapter:%p called w/o HwContext - Bus=%d Slot=%d PNP=%d\n",
                       DeviceExtension, busNumber, slotNumber, ConfigInfo->NumberOfAccessRanges != 0);
#endif
    }
...
//
BOOLEAN NTAPI HwInitialize(IN PVOID DeviceExtension)
{
    PHW_DEVICE_EXTENSION DevExt = (PHW_DEVICE_EXTENSION)DeviceExtension;
...
//
BOOLEAN NTAPI HwStartIo(IN PVOID DeviceExtension, IN PSCSI_REQUEST_BLOCK Srb)
{
    PHW_DEVICE_EXTENSION DevExt = (PHW_DEVICE_EXTENSION)DeviceExtension;
...
VOID NTAPI FallbackTimer(IN PVOID DeviceExtension)
{
    PHW_DEVICE_EXTENSION DevExt = (PHW_DEVICE_EXTENSION)DeviceExtension;
...
//
BOOLEAN NTAPI HwInterrupt(IN PVOID DeviceExtension)
{
    PHW_DEVICE_EXTENSION DevExt = (PHW_DEVICE_EXTENSION)DeviceExtension;
...
//
BOOLEAN NTAPI HwResetBus(IN PVOID DeviceExtension, IN ULONG PathId)
{
...
//
SCSI_ADAPTER_CONTROL_STATUS NTAPI HwAdapterControl(
    IN PVOID DeviceExtension,
