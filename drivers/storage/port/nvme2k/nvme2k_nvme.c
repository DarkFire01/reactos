...
BOOLEAN NvmeInitializeController(IN PHW_DEVICE_EXTENSION DevExt)
{
...
#endif
    {
        ...
        while (!DevExt->InitComplete && pollCount < 10000) {  // 10 second timeout
            ...
            pollCount++;
            /* Progress every 1 second so we can see if we're stuck or making progress */
            if ((pollCount % 1000) == 0 && pollCount > 0) {
                ScsiDebugPrint(0, "nvme2k: NvmeInitializeController - still polling for init, %u sec\n", pollCount / 1000);
            }
        }

        if (!DevExt->InitComplete) {
            ScsiDebugPrint(0, "nvme2k: NvmeInitializeController - TIMEOUT waiting for init completion after %u ms!\n", pollCount);
            return FALSE;
        }
    }
