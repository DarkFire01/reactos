...
/*
 * Each hook type has a slot. The slot of a driver's hook type gets every OID
 * completion and status indication of the driver's adapters. The unhooked
 * slot passes them straight on, a WDI slot hands them to the upper edge once
 * it has attached through NMR.
 */
