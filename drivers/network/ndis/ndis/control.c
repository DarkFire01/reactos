  ...
  if (fWrite) {
    if (Lock->Context == PsGetCurrentThread()) {
      ...
      for (ProcessorNumber = KeNumberProcessors; ProcessorNumber--; ) {
        if (ProcessorNumber != KeGetCurrentProcessorIndex()) {
    ...
    RefCount = InterlockedIncrement((PLONG)&Lock->RefCount[KeGetCurrentProcessorIndex()].RefCount);
    /* Racing with a exclusive write lock case. */
    if (Lock->SpinLock != 0) {
      if (RefCount == 1) {
        if (Lock->Context != PsGetCurrentThread()) {
          /* Wait for the exclusive lock to be released. */
          Lock->RefCount[KeGetCurrentProcessorIndex()].RefCount--;
 ...
 */
{
  ...
  /* A zero wait means wait indefinitely, not poll */
  if (MsToWait)
  {
  ...
  Status = KeWaitForSingleObject(&Event->Event, Executive, KernelMode, FALSE, TimeoutPtr);
