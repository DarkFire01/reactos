 ...
 * NET_BUFFER's MDL chain as is. The length is recomputed because callers may
 * leave Private.TotalLength stale.
 */
VOID
NTAPI
...
/**
 * @brief Translate packets from the cursor onwards into one NET_BUFFER_LIST chain.
 * A run stops at the first packet whose NDIS_FLAGS_DONT_LOOPBACK differs.
 * @return TRUE when packets remain.
 */
BOOLEAN
NTAPI
