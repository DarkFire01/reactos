...
/*
 * Clients and providers of the same NPI get bound to each other. The client
 * decides whether a binding happens, from its ClientAttachProvider callback,
 * by calling NmrClientAttachProvider. Undoing one detaches the client first,
 * then the provider, then cleans up both.
 *
 * Nothing here is initialized at run time, since a driver that imports NETIO
 * can call in before NETIO's own entry point ever runs.
 */
