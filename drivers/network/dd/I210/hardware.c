...
NICAllocateIoResources(IN PI21X_ADAPTER Adapter)
{
    ...
    /*NdisMAllocateSharedMemory(Adapter->MiniportAdapterHandle,
                              sizeof(EI21XTRANSMIT_DESCRIPTOR) * NUM_TRANSMIT_DESCRIPTORS,
                              FALSE,
    ...
    {
        PEI21XTRANSMIT_DESCRIPTOR Descriptor = Adapter->TransmitDescriptors + n;
        Descriptor->Address = 0;
        Descriptor->Length = 0;
    }

    NdisMAllocateSharedMemory(Adapter->MiniportAdapterHandle,
                              sizeof(EI21XRECEIVE_DESCRIPTOR) * NUM_RECEIVE_DESCRIPTORS,
                              FALSE,
    ...
    {
        PEI21XRECEIVE_DESCRIPTOR Descriptor = Adapter->ReceiveDescriptors + n;
...
NICReleaseIoResources(IN PI21X_ADAPTER Adapter)
{
    ...
    /*if (Adapter->ReceiveDescriptors != NULL)
    {
        ...
        if (Adapter->IoBase)
        {
            I21XWriteUlong(Adapter, I21X_REG_RDH, 0);
            I21XWriteUlong(Adapter, I21X_REG_RDT, 0);
        }

        NdisMFreeSharedMemory(Adapter->MiniportAdapterHandle,
                              sizeof(I21X_RECEIVE_DESCRIPTOR) * NUM_RECEIVE_DESCRIPTORS,
                              FALSE,
    ...
    if (Adapter->TransmitDescriptors != NULL)
    {
        ...
        if (Adapter->IoBase)
        {
            I21XWriteUlong(Adapter, I21X_REG_TDH, 0);
            I21XWriteUlong(Adapter, I21X_REG_TDT, 0);
        }

        NdisMFreeSharedMemory(Adapter->MiniportAdapterHandle,
                              sizeof(I21X_TRANSMIT_DESCRIPTOR) * NUM_TRANSMIT_DESCRIPTORS,
                              FALSE,
...
NICSoftReset(IN PI21X_ADAPTER Adapter)
{
    ...
    for (ULONG ResetAttempts = 0; ResetAttempts < MAX_RESET_ATTEMPTS; ResetAttempts++)
    {
        ...
        /*I21XReadUlong(Adapter, ???, &Value);
        if (!(Value & I21X_CTRL_RST))
        {
                          ...
                          OUT PUCHAR MacAddress)
{
    ...
    {
        if (!I21XReadEeprom(Adapter, (UCHAR)n, &AddressWord))
        {
            return NDIS_STATUS_FAILURE;
        }
...
NICUpdateMulticastList(IN PI21X_ADAPTER Adapter)
{
    ...
    for (UINT n = 0; n < MAXIMUM_MULTICAST_ADDRESSES; ++n)
    {
        ...
        if (Rah || Ral)
        {
            ...
            I21XWriteUlong(Adapter, I21X_REG_RAL + (8*n), Ral);
            I21XWriteUlong(Adapter, I21X_REG_RAH + (8*n), Rah);
        }
        else
        {
            I21XWriteUlong(Adapter, I21X_REG_RAH + (8*n), 0);
            I21XWriteUlong(Adapter, I21X_REG_RAL + (8*n), 0);
        }*/
    }
...
NICApplyPacketFilter(IN PI21X_ADAPTER Adapter)
{
    ...
    /*I21XReadUlong(Adapter, I21X_REG_RCTL, &FilterMask);

    FilterMask &= ~I21X_RCTL_FILTER_BITS;
    FilterMask |= PacketFilterToMask(Adapter->PacketFilter);
    I21XWriteUlong(Adapter, I21X_REG_RCTL, FilterMask);
...
NICUpdateLinkStatus(IN PI21X_ADAPTER Adapter)
{
    ...
    /*I21XReadUlong(Adapter, I21X_REG_STATUS, &DeviceStatus);
    Adapter->MediaState = (DeviceStatus & I21X_STATUS_LU) ?
                           NdisMediaStateConnected :
                           NdisMediaStateDisconnected;
    SpeedIndex = (DeviceStatus & I21X_STATUS_SPEEDMASK) >> I21X_STATUS_SPEEDSHIFT;
                ...
                USHORT *Result)
{
    ...
    for (UINT n = 0; n < MAX_EEPROM_READ_ATTEMPTS; n++)
    {
        ...
        // TODO
        // I21XReadEeprom(Adapter, ???, &Value);

        /*if (Value & I21X_EERD_DONE)
        {
            break;
        }*/
    }
    ...
    //*Result = (USHORT)(Value >> I21X_EERD_DATA_SHIFT);
    return TRUE;
}
