...
PropertyHandler_InputVolume(IN PPCPROPERTY_REQUEST PropertyRequest)
{
    ...
    ULONG NodeCount = 0;
    PULONG Nodes = 0;
    Status = Node->GetNodesWithType(0x01, &NodeCount, &Nodes);
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("HDAUDIO: GetNodesWithType failed with %x\n", Status);
        Miniport->Release();
    ...
    for (ULONG NodeIndex = 0; NodeIndex < NodeCount; NodeIndex++)
    {
        ...
        Status = Node->GetAmplifierDetails(Nodes[NodeIndex], TRUE, &AmplifierCapabilities);
        if (!NT_SUCCESS(Status))
        {
        ...
        if (AmplifierCapabilities.NumSteps == 0 || AmplifierCapabilities.Steps == 0)
        {
            ...
            // use fallback values
            AmplifierCapabilities.NumSteps = Node->GetCachedOutputGain();
            AmplifierCapabilities.Steps = 2;
        }

        PNODE_CONTEXT NodeContext = Node->FindNodeId(Nodes[NodeIndex]);
        if (!NodeContext)
        {
            DPRINT1("HDAUDIO: No node context for node %u\n", Nodes[NodeIndex]);
        ...
        else if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
        {
            ...
            DPRINT("Node %d, Gain %d\n", Nodes[NodeIndex], Gain);
        ...
        else if (PropertyRequest->Verb & KSPROPERTY_TYPE_SET)
        {
            ...
            Set.Mute = 0x0;
            Set.Gain = Gain;
            Status = Node->SetAmplifierGainMute(Nodes[NodeIndex], &Set);
            DPRINT("SetAmplifierGainMute Status %x, Node %d, Gain %d\n", Status, Nodes[NodeIndex], Gain);
            Set.Output = 0x0;
            Set.Input = 0x1;
            Set.Left = 0x0;
            Set.Right = 0x1;
...
PropertyHandler_InputMute(IN PPCPROPERTY_REQUEST PropertyRequest)
{
    ...
    for (ULONG NodeIndex = 0; NodeIndex < NodeCount; NodeIndex++)
    {
        ...
        if (PropertyRequest->Verb & KSPROPERTY_TYPE_BASICSUPPORT)
        {
            if (PropertyRequest->ValueSize >= sizeof(KSPROPERTY_DESCRIPTION))
            {
                ...
                if (PropertyRequest->ValueSize >= BasicPropertySize)
                {
                    ...
                    for (ULONG Channel = 0; Channel < NodeContext->ChannelCount; Channel++)
                    {
                        Range[Channel].Bounds.SignedMinimum = 0;
                        Range[Channel].Bounds.SignedMaximum = 1;
        ...
        else if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
        {
            ...
            DPRINT("Node %d, Mute %x\n", Nodes[NodeIndex], Mute);
            *Value = Mute;
...
PropertyHandler_OutputVolume(IN PPCPROPERTY_REQUEST PropertyRequest)
{
    ...
    ULONG NodeCount = 0;
    PULONG Nodes = 0;
    Status = Node->GetNodesWithType(0x00, &NodeCount, &Nodes);
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("HDAUDIO: GetNodesWithType failed with %x\n", Status);
        Miniport->Release();
    ...
    {
        AMPLIFIER_CAPABILITIES AmplifierCapabilities;
        Status = Node->GetAmplifierDetails(Nodes[NodeIndex], FALSE, &AmplifierCapabilities);
        if (!NT_SUCCESS(Status))
        {
            DPRINT1("HDAUDIO: GetAmplifierDetails failed status %x, Node %d\n", Status, Nodes[NodeIndex]);
            Status = STATUS_SUCCESS;
