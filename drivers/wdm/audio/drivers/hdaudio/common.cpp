...
CAdapterCommon::Initialize(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PRESOURCELIST ResourceList)
{
    ...
    if (VendorId != 0x8086)
    {
        ...
        // Turn on node amplifiers
        for (ULONG PinNodeIndex = 0; PinNodeIndex < PinNodeCount; PinNodeIndex++)
        {
            AMPLIFIER_CAPABILITIES AmplifierCapabilities;
            Status = OutNode->GetAmplifierDetails(PinNodes[PinNodeIndex], FALSE, &AmplifierCapabilities);
            if (!NT_SUCCESS(Status))
            {
                DPRINT1("HDAUDIO: GetAmplifierDetails failed %x for node %u\n", Status, PinNodes[PinNodeIndex]);
            }
            ...
            Set.Index = 0;
            Set.Mute = 0x0;
            Set.Gain = AmplifierCapabilities.NumSteps ? AmplifierCapabilities.NumSteps : Gain;
            Status = OutNode->SetAmplifierGainMute(PinNodes[PinNodeIndex], &Set);
            if (!NT_SUCCESS(Status))
            {
                DPRINT1("HDAUDIO: SetAmplifierGainMute failed %x for node %u\n", Status, PinNodes[PinNodeIndex]);
            }
        }
    }
