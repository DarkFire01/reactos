...
typedef struct {
    ...
    pHalGetAcpiTable_win10                HalpGetAcpiTable;
...
#define HAL_ACPI_DISPATCH_SIGNATURE_WIN10   'HAL '
#define HAL_ACPI_DISPATCH_VERSION_WIN10     4
...
#define HalGetAcpiTable             ((pHalGetAcpiTable_win10)PmHalDispatchTable->Function[HalGetAcpiTable])
...
} ACPI_HAL_DISPATCH_TABLE_WIN10, *PACPI_HAL_DISPATCH_TABLE_WIN10;

#define ACPI_HAL_DISPATCH_SIGNATURE_WIN10   'ACPI'
#define ACPI_HAL_DISPATCH_VERSION_WIN10     2
...
VOID
AcpiiEnableDisableGPEvents(_In_ BOOLEAN Enable)
{
    UNIMPLEMENTED;
}

VOID
AcpiiInitEnableAcpi(_In_ BOOLEAN ReEnable)
{
    DPRINT("AcpiiInitEnableAcpi: Entry %X\n",  ACPIInitUACPI());
    UNIMPLEMENTED;
}

VOID
AcpiiGpeEnableWakeEvents(VOID)
{
    UNIMPLEMENTED;
}

VOID
AcpiiMarkHiberPhase(VOID)
{
    UNIMPLEMENTED;
}
...
{
    ACPI_HAL_DISPATCH_TABLE_WIN10 GlobalAcpiToHalDispatchTable;
    GlobalAcpiToHalDispatchTable.Signature = ACPI_HAL_DISPATCH_SIGNATURE_WIN10;
#if 1
    GlobalAcpiToHalDispatchTable.Version = 1;
#else
    GlobalAcpiToHalDispatchTable.Version = ACPI_HAL_DISPATCH_VERSION_WIN10;
#endif
    /* NT5 */
    GlobalAcpiToHalDispatchTable.AcpipEnableDisableGPEvents = AcpiiEnableDisableGPEvents;
    GlobalAcpiToHalDispatchTable.AcpipInitEnableAcpi = AcpiiInitEnableAcpi;
    GlobalAcpiToHalDispatchTable.AcpipGpeEnableWakeEvents = AcpiiGpeEnableWakeEvents;
#if 0
    /* NT6 */
    GlobalAcpiToHalDispatchTable.AcpipMarkHiberPhase = AcpiiMarkHiberPhase;
#endif
    PmAcpiDispatchTable = (PPM_DISPATCH_TABLE)&GlobalAcpiToHalDispatchTable;
    HalInitPowerManagement(PmAcpiDispatchTable, &PmHalDispatchTable);
}
