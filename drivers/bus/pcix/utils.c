                     ...
                     IN BOOLEAN ForPowerDown)
{
    ...
    if (ForPowerDown)
    {
        ...
        if ((KeepPower) || (HackFlags & PCI_HACK_DONT_DISABLE_DECODES))
            return FALSE;
    }
    ...
    /* Critical devices, like interrupt controllers, must keep decoding */
    return !(HackFlags & PCI_HACK_CRITICAL_DEVICE);
}
