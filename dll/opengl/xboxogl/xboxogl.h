/*
 * PROJECT:     Xbox NV2A OpenGL 1.x Installable Client Driver (scaffold)
 * LICENSE:     GPL-2.0-or-later
 * PURPOSE:     Shared types
 *
 * This DLL is loaded by opengl32.dll when the xboxdisp display driver claims
 * the active HDC.  It implements the WGL ICD ABI and forwards immediate-mode
 * geometry through GDI, which the display driver routes to the NV2A 2D engine
 * (solid colour fill) or to ENG_BitBlt (DIB swap).
 *
 * Scope (matches the per-session contract):
 *   - glClear, glClearColor (real, via PatBlt with a solid brush)
 *   - glBegin / glEnd / glVertex / glColor (immediate mode, flat-shaded
 *     triangles and quads rendered via GDI Polygon())
 *   - glViewport, glOrtho, glLoadIdentity, glMatrixMode, glPushMatrix,
 *     glPopMatrix (state stored, transform applied at vertex submission)
 *   - glEnable, glDisable, glFinish, glFlush (no-op success)
 *   - glGetString returns honest identification
 *
 * All other entry points present in the WGL dispatch table are no-op stubs
 * so an OpenGL application can load without unresolved entry-point errors.
 */
