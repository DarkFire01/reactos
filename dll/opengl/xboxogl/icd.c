 ...
 * PURPOSE:     WGL ICD Drv* entry points
 *
 * opengl32.dll discovers us via the display driver's OPENGL_GETINFO escape
 * ("xboxopengl") and then loads this DLL from
 *   HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\OpenGLDrivers\xboxopengl
 * The .inf shipped by the miniport writes that registry value.
 *
 * Pixel formats: we publish a single 32 bpp RGBA single-buffered format.
 * Double-buffered support would require allocating a back DIB; that's left
...
/* We MUST advertise a double-buffered format.  opengl32 routes an app to this
 * ICD only when the ICD has a pixel format matching the app's request; if the
 * app asks for PFD_DOUBLEBUFFER (e.g. glxgears) and we offer only single-
 * buffered, opengl32 silently falls back to its GENERIC software renderer
 * (Mesa) and bypasses the NV2A entirely.  Our present model (render to an
 * offscreen surface, blit on SwapBuffers) IS double-buffered, so this is honest.
 *   format 1 = double-buffered, format 2 = single-buffered.
 * Depth/stencil is 24/8 to match the miniport's Z24S8 zeta surface (it falls
 * back to Z16 only if the contiguous depth allocation fails). */
