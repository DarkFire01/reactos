...
#define OVERRIDE(field, fn) g_xglDispatchTable.glDispatchTable.field = (void*)(fn)
    ...
    /* GL 1.2 entry points (glDrawRangeElements, glTexImage3D, glBlendColor,
     * glBlendEquation, ...) are NOT part of the GL 1.1 GLCLTPROCTABLE; apps
     * reach them through wglGetProcAddress -> DrvGetProcAddress (see icd.c). */
