    ...
    PULONG lpBytesReturned)
{
    ...
    /* Wait for the I/O to complete, staying alertable so queued completion routines still run */
    if (!IoResult)
    {
        do
        {
            WaitResult = WaitForSingleObjectEx(Overlapped.hEvent, INFINITE, TRUE);
        } while (WaitResult == WAIT_IO_COMPLETION);
    }

    /* The request is no longer pending here, so this only collects the status */
