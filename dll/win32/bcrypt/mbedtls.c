...
#undef LOAD_FUNCPTR
#endif
    /* Only set the contexts up here. Seeding is left to the first caller that
       needs randomness, because it draws entropy through CryptGenRandom,
       which loads a cryptographic service provider - far more than may be
       done from DllMain. rsaenh imports us by way of crypt32, so seeding here
       re-enters it before its own DLL_PROCESS_ATTACH has initialised the
       handle table, and its lock is still zeroed. */
...
/* The random source every caller here draws from. Seeds the generator on the
   first call, so that nothing is drawn while the loader lock is held. */
