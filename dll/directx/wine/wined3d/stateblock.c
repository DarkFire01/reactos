...
#ifdef __REACTOS__
static void stateblock_init_lights(struct wined3d_stateblock *stateblock, const struct wine_rb_tree *src_tree)
{
    struct wine_rb_tree *dst_tree = &stateblock->stateblock_state.light_state->lights_tree;
    struct wined3d_light_info *src_light;

    WINE_RB_FOR_EACH_ENTRY(src_light, src_tree, struct wined3d_light_info, entry)
    {
        struct wined3d_light_info *dst_light = heap_alloc(sizeof(*dst_light));

        *dst_light = *src_light;
        wine_rb_put(dst_tree, (void *)(ULONG_PTR)dst_light->OriginalIndex, &dst_light->entry);
...
#ifdef __REACTOS__
    WINE_RB_FOR_EACH_ENTRY_DESTRUCTOR(light, cursor, &state->light_state->lights_tree, struct wined3d_light_info, entry)
    {
        if (light->changed)
            list_remove(&light->changed_entry);
        wine_rb_remove(&state->light_state->lights_tree, &light->entry);
        heap_free(light);
    }
#else
...
struct wined3d_light_info *wined3d_light_state_get_light(const struct wined3d_light_state *state, unsigned int idx)
{
    ...
    if (!(entry = wine_rb_get(&state->lights_tree, (void *)(ULONG_PTR)idx)))
        return NULL;

    return WINE_RB_ENTRY_VALUE(entry, struct wined3d_light_info, entry);
}
#else
struct wined3d_light_info *wined3d_light_state_get_light(const struct wined3d_light_state *state, unsigned int idx)
{
...
#ifdef __REACTOS__
        wine_rb_put(&state->lights_tree, (void *)(ULONG_PTR)light_idx, &object->entry);
#else
        ...
        const struct wined3d_light_state *src_state)
{
     ...
     * differ. */
#ifdef __REACTOS__
    WINE_RB_FOR_EACH_ENTRY(dst, &dst_state->lights_tree, struct wined3d_light_info, entry)
#else
    WINE_RB_FOR_EACH_ENTRY(dst, &dst_state->lights_tree, struct wined3d_light_info, entry)
#endif
    {
...
#ifdef __REACTOS__
static int lights_compare(const void *key, const struct wine_rb_entry *entry)
{
    const struct wined3d_light_info *light = WINE_RB_ENTRY_VALUE(entry, struct wined3d_light_info, entry);
...
#ifdef __REACTOS__
    wine_rb_init(&state->light_state.lights_tree, lights_compare);
#else
