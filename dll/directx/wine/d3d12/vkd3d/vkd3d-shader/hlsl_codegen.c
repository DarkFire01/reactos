...
{
    struct wine_rb_tree var_defs;
};

static int copy_propagation_var_def_compare(const void *key, const struct wine_rb_entry *entry)
{
    struct copy_propagation_var_def *var_def = WINE_RB_ENTRY_VALUE(entry, struct copy_propagation_var_def, entry);
    uintptr_t key_int = (uintptr_t)key, entry_int = (uintptr_t)var_def->var;

    return (key_int > entry_int) - (key_int < entry_int);
}

static void copy_propagation_var_def_destroy(struct wine_rb_entry *entry, void *context)
{
    struct copy_propagation_var_def *var_def = WINE_RB_ENTRY_VALUE(entry, struct copy_propagation_var_def, entry);
...
{
    struct wine_rb_entry *entry = wine_rb_get(&state->var_defs, var);

    if (entry)
        return WINE_RB_ENTRY_VALUE(entry, struct copy_propagation_var_def, entry);
    else
        return NULL;
}
        ...
        struct copy_propagation_state *state, struct hlsl_ir_var *var)
{
    ...
    res = wine_rb_put(&state->var_defs, var, &var_def->entry);
    assert(!res);
...
static bool copy_propagation_execute(struct hlsl_ctx *ctx, struct hlsl_block *block)
{
    ...
    wine_rb_init(&state.var_defs, copy_propagation_var_def_compare);

    progress = copy_propagation_transform_block(ctx, block, &state);

    wine_rb_destroy(&state.var_defs, copy_propagation_var_def_destroy, NULL);
...
static void dump_function(struct wine_rb_entry *entry, void *context)
{
    ...
    wine_rb_for_each_entry(&func->overloads, dump_function_decl, ctx);
}
        ...
        enum vkd3d_shader_target_type target_type, struct vkd3d_shader_code *out)
{
    ...
    if (TRACE_ON())
        wine_rb_for_each_entry(&ctx->functions, dump_function, ctx);
