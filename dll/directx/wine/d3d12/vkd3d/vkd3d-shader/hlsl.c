...
{
    return wine_rb_get(&ctx->functions, name) != NULL;
}
...
struct hlsl_ir_function_decl *hlsl_get_func_decl(struct hlsl_ctx *ctx, const char *name)
{
    ...
    if ((entry = wine_rb_get(&ctx->functions, name)))
    {
...
bool hlsl_scope_add_type(struct hlsl_scope *scope, struct hlsl_type *type)
{
    ...
    wine_rb_put(&scope->types, type->name, &type->scope_entry);
    return true;
}
...
static void free_function_rb(struct wine_rb_entry *entry, void *context)
{
...
void hlsl_add_function(struct hlsl_ctx *ctx, char *name, struct hlsl_ir_function_decl *decl, bool intrinsic)
{
    ...
    func_entry = wine_rb_get(&ctx->functions, name);
    if (func_entry)
    {
    ...
    wine_rb_put(&func->overloads, decl->parameters, &decl->entry);
    func->intrinsic = intrinsic;
    wine_rb_put(&ctx->functions, func->name, &func->entry);
}
        ...
        const struct hlsl_profile_info *profile, struct vkd3d_shader_message_context *message_context)
{
    ...
    wine_rb_init(&ctx->functions, compare_function_rb);
...
static void hlsl_ctx_cleanup(struct hlsl_ctx *ctx)
{
    ...
    wine_rb_destroy(&ctx->functions, free_function_rb, NULL);
