...
struct vkd3d_spirv_builder
{
    ...
    struct wine_rb_tree declarations;
...
static int vkd3d_spirv_declaration_compare(const void *key, const struct wine_rb_entry *e)
{
    const struct vkd3d_spirv_declaration *a = key;
    const struct vkd3d_spirv_declaration *b = WINE_RB_ENTRY_VALUE(e, const struct vkd3d_spirv_declaration, entry);
    int ret;
...
static void vkd3d_spirv_declaration_free(struct wine_rb_entry *entry, void *context)
{
    struct vkd3d_spirv_declaration *d = WINE_RB_ENTRY_VALUE(entry, struct vkd3d_spirv_declaration, entry);
        ...
        const struct vkd3d_spirv_declaration *declaration)
{
    ...
    if (wine_rb_put(&builder->declarations, d, &d->entry) == -1)
    {
        ...
        SpvOp op, uint32_t operand0, vkd3d_spirv_build1_pfn build_pfn)
{
    ...
    if ((entry = wine_rb_get(&builder->declarations, &declaration)))
        return WINE_RB_ENTRY_VALUE(entry, struct vkd3d_spirv_declaration, entry)->id;
...
static void vkd3d_spirv_builder_init(struct vkd3d_spirv_builder *builder, const char *entry_point)
{
    ...
    wine_rb_init(&builder->declarations, vkd3d_spirv_declaration_compare);
...
static void vkd3d_spirv_builder_free(struct vkd3d_spirv_builder *builder)
{
    ...
    wine_rb_destroy(&builder->declarations, vkd3d_spirv_declaration_free, NULL);
...
static int vkd3d_symbol_compare(const void *key, const struct wine_rb_entry *entry)
{
    const struct vkd3d_symbol *a = key;
    const struct vkd3d_symbol *b = WINE_RB_ENTRY_VALUE(entry, const struct vkd3d_symbol, entry);
    int ret;
...
static void vkd3d_symbol_free(struct wine_rb_entry *entry, void *context)
{
    struct vkd3d_symbol *s = WINE_RB_ENTRY_VALUE(entry, struct vkd3d_symbol, entry);
...
struct vkd3d_dxbc_compiler
{
    ...
    struct wine_rb_tree symbol_table;
    uint32_t temp_id;
        ...
        struct vkd3d_shader_message_context *message_context, const struct vkd3d_shader_location *location)
{
    ...
    wine_rb_init(&compiler->symbol_table, vkd3d_symbol_compare);
        ...
        const struct vkd3d_symbol *symbol)
{
    ...
    if (wine_rb_put(&compiler->symbol_table, s, &s->entry) == -1)
    {
        ...
        const struct vkd3d_shader_register *reg, struct vkd3d_shader_register_info *register_info)
{
    ...
    if (!(entry = wine_rb_get(&compiler->symbol_table, &reg_symbol)))
    {
    ...
    symbol = WINE_RB_ENTRY_VALUE(entry, struct vkd3d_symbol, entry);
        ...
        enum vkd3d_shader_interpolation_mode interpolation_mode)
{
    ...
    SpvStorageClass storage_class;
    struct wine_rb_entry *entry = NULL;
            ...
            && (reg->type == VKD3DSPR_INCONTROLPOINT || reg->type == VKD3DSPR_PATCHCONST))
    {
        ...
        if ((entry = wine_rb_get(&compiler->symbol_table, &tmp_symbol)))
        {
            symbol = WINE_RB_ENTRY_VALUE(entry, struct vkd3d_symbol, entry);
            tmp_symbol = *symbol;
        ...
        bool is_uav_counter, struct vkd3d_descriptor_variable_info *var_info)
{
    ...
    if ((entry = wine_rb_get(&compiler->symbol_table, &symbol)))
    {
        var_info->array_symbol = WINE_RB_ENTRY_VALUE(entry, struct vkd3d_symbol, entry);
        ...
        const struct vkd3d_shader_phase *phase)
{
    ...
    if (is_control_point_phase(phase))
    {
        ...
        for (i = 0; i < signature->element_count; ++i)
        {
            ...
            if ((entry = wine_rb_get(&compiler->symbol_table, &reg_symbol)))
            {
                wine_rb_remove(&compiler->symbol_table, entry);
                ...
                if (wine_rb_put(&compiler->symbol_table, symbol, entry) == -1)
                {
...
static void vkd3d_dxbc_compiler_emit_hull_shader_input_initialisation(struct vkd3d_dxbc_compiler *compiler)
{
    ...
    for (i = 0; i < signature->element_count; ++i)
    {
        ...
        entry = wine_rb_get(&compiler->symbol_table, &symbol_key);
        symbol = WINE_RB_ENTRY_VALUE(entry, struct vkd3d_symbol, entry);
        ...
        const struct vkd3d_shader_register *resource_reg)
{
    ...
    entry = wine_rb_get(&compiler->symbol_table, &resource_key);
    assert(entry);
    return WINE_RB_ENTRY_VALUE(entry, struct vkd3d_symbol, entry);
}
        ...
        const struct vkd3d_shader_register *resource_reg, const struct vkd3d_shader_register *sampler_reg)
{
    ...
    if (sampler_reg)
    {
        ...
        if (!(entry = wine_rb_get(&compiler->symbol_table, &key)))
            return NULL;
        sampler_symbol = WINE_RB_ENTRY_VALUE(entry, struct vkd3d_symbol, entry);
...
void vkd3d_dxbc_compiler_destroy(struct vkd3d_dxbc_compiler *compiler)
{
    ...
    wine_rb_destroy(&compiler->symbol_table, vkd3d_symbol_free, NULL);
