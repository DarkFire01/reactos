...
static const struct hlsl_ir_function_decl *get_func_decl(struct wine_rb_tree *funcs, char *name, struct list *params)
{
    struct hlsl_ir_function *func;
    struct wine_rb_entry *entry;

    if ((entry = wine_rb_get(funcs, name)))
    {
        func = RB_ENTRY_VALUE(entry, struct hlsl_ir_function, entry);

        if ((entry = wine_rb_get(&func->overloads, params)))
        ...
        const char *name, const struct parse_initializer *params)
{
    ...
    wine_rb_for_each_entry(&func->overloads, find_function_call_exact, &args);
    if (!args.decl)
