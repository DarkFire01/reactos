...
{
#ifdef __REACTOS__
    struct wine_rb_entry node_map;
#else
    struct wine_rb_tree node_map;
#endif
...
static int uia_runtime_id_compare(const void *key, const struct wine_rb_entry *entry)
{
    struct uia_provider_thread_map_entry *prov_entry = WINE_RB_ENTRY_VALUE(entry, struct uia_provider_thread_map_entry, entry);
...
void uia_provider_thread_remove_node(HUIANODE node)
{
    ...
    if (!list_empty(&node_data->node_map_list_entry))
    {
        ...
        {
            wine_rb_remove(&provider_thread.node_map, &node_data->map->entry);
...
static void uia_provider_thread_disconnect_node(SAFEARRAY *sa)
{
    ...
    rb_entry = wine_rb_get(&provider_thread.node_map, sa);
    if (rb_entry)
    {
        ...
        prov_map = WINE_RB_ENTRY_VALUE(rb_entry, struct uia_provider_thread_map_entry, entry);
        ...
        wine_rb_remove(&provider_thread.node_map, &prov_map->entry);
...
static HRESULT uia_provider_thread_add_node(HUIANODE node, SAFEARRAY *rt_id)
{
    ...
    if (rt_id)
    {
        ...
        else
        {
            ...
            wine_rb_put(&provider_thread.node_map, prov_map->runtime_id, &prov_map->entry);
        }
...
static BOOL uia_start_provider_thread(void)
{
    ...
    if (++provider_thread.ref == 1)
    {
        ...
        wine_rb_init(&provider_thread.node_map, uia_runtime_id_compare);
