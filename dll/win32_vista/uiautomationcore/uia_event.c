...
{
#ifdef __REACTOS__
    struct wine_rb_entry event_map;
#else
...
#ifdef __REACTOS__
    struct wine_rb_entry serverside_event_map;
#else
...
static int uia_serverside_event_id_compare(const void *key, const struct wine_rb_entry *entry)
{
    struct uia_event *event = WINE_RB_ENTRY_VALUE(entry, struct uia_event, u.serverside.serverside_event_entry);
...
{
    struct uia_event_map_entry *event_entry = WINE_RB_ENTRY_VALUE(entry, struct uia_event_map_entry, entry);
...
static struct uia_event_map_entry *uia_get_event_map_entry_for_event(int event_id)
{
    ...
    if (uia_event_map.event_count && (rb_entry = wine_rb_get(&uia_event_map.event_map, &event_id)))
        map_entry = WINE_RB_ENTRY_VALUE(rb_entry, struct uia_event_map_entry, entry);
...
static HRESULT uia_event_map_add_event(struct uia_event *event)
{
    ...
    if (!(event_entry = uia_get_event_map_entry_for_event(event->event_id)))
    {
        ...
        if (!uia_event_map.event_count)
            wine_rb_init(&uia_event_map.event_map, uia_event_map_id_compare);
        wine_rb_put(&uia_event_map.event_map, &event->event_id, &event_entry->entry);
...
static void uia_event_map_entry_release(struct uia_event_map_entry *entry)
{
    ...
    if (!ref)
    {
        ...
        wine_rb_remove(&uia_event_map.event_map, &entry->entry);
        uia_event_map.event_count--;
        LeaveCriticalSection(&event_map_cs);
...
static ULONG WINAPI uia_event_Release(IWineUiaEvent *iface)
{
    ...
    if (!ref)
    {
        ...
        {
            EnterCriticalSection(&event_map_cs);
            wine_rb_remove(&uia_event_map.serverside_event_map, &event->u.serverside.serverside_event_entry);
...
HRESULT create_serverside_uia_event(struct uia_event **out_event, LONG process_id, LONG event_cookie)
{
    ...
    *out_event = NULL;
    EnterCriticalSection(&event_map_cs);
    if (uia_event_map.serverside_event_count && (rb_entry = wine_rb_get(&uia_event_map.serverside_event_map, &event_identifier)))
    {
        *out_event = WINE_RB_ENTRY_VALUE(rb_entry, struct uia_event, u.serverside.serverside_event_entry);
        hr = S_FALSE;
        goto exit;
    }
    ...
    if (uia_event_map.serverside_event_count == 1)
        wine_rb_init(&uia_event_map.serverside_event_map, uia_serverside_event_id_compare);
    wine_rb_put(&uia_event_map.serverside_event_map, &event_identifier, &event->u.serverside.serverside_event_entry);
    *out_event = event;
