...
{
#ifdef __REACTOS__
    struct wine_rb_entry handler_map;
#else
...
static int uia_com_event_handler_id_compare(const void *key, const struct wine_rb_entry *entry)
{
    struct uia_event_handler_map_entry *map_entry = WINE_RB_ENTRY_VALUE(entry, struct uia_event_handler_map_entry, entry);
        ...
        struct uia_com_event *event)
{
    ...
    if (!com_event_handlers.handler_count)
        wine_rb_init(&com_event_handlers.handler_map, uia_com_event_handler_id_compare);

    if ((rb_entry = wine_rb_get(&com_event_handlers.handler_map, &event_ident)))
        event_map = WINE_RB_ENTRY_VALUE(rb_entry, struct uia_event_handler_map_entry, entry);
    else
    {
        ...
        wine_rb_put(&com_event_handlers.handler_map, &event_ident, &event_map->entry);
    }
...
static void uia_event_handler_map_entry_destroy(struct uia_event_handler_map_entry *entry)
{
    ...
    wine_rb_remove(&com_event_handlers.handler_map, &entry->entry);
...
static void uia_event_handlers_remove_handlers(IUnknown *handler_iface, SAFEARRAY *runtime_id, int event_id)
{
    ...
    if (com_event_handlers.handler_count && (rb_entry = wine_rb_get(&com_event_handlers.handler_map, &event_ident)))
        uia_event_handler_map_entry_destroy(WINE_RB_ENTRY_VALUE(rb_entry, struct uia_event_handler_map_entry, entry));
