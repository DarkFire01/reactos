 ...
 * PURPOSE:     Transaction, boot allocation and conflict query actions
 ...
 * through the arbiter's UnpackRequirement callback and classifies
 * it.
 *
 * @param[in] Arbiter
    ...
    _Out_ PARBITER_ALTERNATIVE Alternative)
{
     ...
     * assign, rather than aliasing the (possibly 32-bit) fields as PUINT64.
     */
    ...
    _In_ ULONG EntryCount)
{
    ...
    /* The final state's Entry stays NULL (the terminator) from the zero-fill. */
 ...
 *
 * @remarks
 * Retest is what makes a multi-arbiter transaction deterministic:
 * after every arbiter's Test has succeeded, each is re-driven
 * through Retest so the exact chosen placements - not a fresh
 * search - are laid down for the Commit that follows.
 */
CODE_SEG("PAGE")
static
NTSTATUS
 ...
 *
 * @remarks
 * Works by repeatedly asking FindSuitableRange to place the
 * resource in the scratch copy: each failure (via the recording
 * callback) names one conflicting owner, which is then removed so
 * the next round finds the next one. Reusing FindSuitableRange
 * means the same share / driver-exclusive / boot semantics apply
 * as at assignment time.
 */
CODE_SEG("PAGE")
static
NTSTATUS
    ...
    _Out_ PARBITER_CONFLICT_INFO *Conflicts)
{
    ...
    *Conflicts = NULL;
    ...
    /* Zero State up front so the Cleanup path is safe on any early failure. */
    RtlZeroMemory(&State, sizeof(State));

    /* Route RtlFindRange conflicts to our recorder for the duration. */
    Arbiter->ConflictCallback = ArbpQueryConflictCallback;
    Arbiter->ConflictCallbackContext = &ConflictingRange;

    /* Work on a scratch copy of the committed allocation. */
    RtlFreeRangeList(Arbiter->PossibleAllocation);
    RtlInitializeRangeList(Arbiter->PossibleAllocation);
    Status = RtlCopyRangeList(Arbiter->PossibleAllocation, Arbiter->Allocation);
    if (!NT_SUCCESS(Status))
    ...
    if (!NT_SUCCESS(IoGetDeviceProperty(PhysicalDeviceObject, DevicePropertyLegacyBusType,
                                        sizeof(Entry.InterfaceType), &Entry.InterfaceType, &ResultLength)))
    {
    ...
    for (;;)
    {
        ...
        if (Count == Capacity)
        {
            PARBITER_CONFLICT_INFO Grown =
                ExAllocatePoolWithTag(PagedPool, (Capacity + 5) * sizeof(ARBITER_CONFLICT_INFO), TAG_ARBITER);
            if (Grown == NULL)
            {
                Status = STATUS_INSUFFICIENT_RESOURCES;
                ExFreePoolWithTag(List, TAG_ARBITER);
