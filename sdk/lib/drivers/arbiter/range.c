 ...
 * The alternative whose priority is advanced. Ordinary priorities
 ...
 * hardware - e.g. the ports the kernel debugger reserves, which
 ...
 *
 * @remarks
 * pci.sys emits a wide-open line-based interrupt requirement
 * (MinimumVector 0, MaximumVector 0xFFFFFFFF) - faithful to
 * Windows, which expects an upstream ACPI _PRT arbiter to clamp it
 * to the routed GSIV. With no ACPI the root IRQ fallback arbiter
 * is the only one in the tree, and RtlFindRange searches top-down,
 * so a loose window resolves to 0xFFFFFFFF - a vector the HAL
 * cannot translate. But the BIOS already routed each device's IRQ
 * (config-space InterruptLine, offset 0x3C), which pci.sys reports
 ...
 * the firmware wired it to, stable across resource rebalances.
 *
 * Interrupt-only by design: the memory/IO/bus windows pci.sys
 * hands the arbiters are exact-fit, so the top-down search already
 * lands correctly and this retention is neither needed nor wanted
 * for them.
 */
CODE_SEG("PAGE")
static
    ...
    _Inout_ PARBITER_ALLOCATION_STATE ArbState)
{
    ...
    {
        /*
         * A vector this PDO already owns (its boot reservation, or a vector a
         * prior pass committed to it) that fits inside the requested window.  The
         * interrupt arbiter records one vector per device, so an owner match is
         * unambiguous.
         */
    ...
    for (;;)
    {
        ...
        else
        {
                 ...
                 ++Alternative)
            {
                ...
                ArbpUpdatePriority(Arbiter, Alternative);
            }
        }
        ...
        /* Identical window to the one that just failed - keep walking. */
        ArbState->CurrentAlternative = Lowest;
    }
}
...
/**
 * @brief
 * Finds a free sub-range of the current candidate window in the
 ...
 * @param[in,out] ArbState
 * The allocation state of the entry being placed. On success,
 * Start and End receive the chosen placement inside the window
 * described by CurrentMinimum and CurrentMaximum.
 *
 * @return
 * Returns TRUE if a placement was found (or granted by one of the
 * conflict escapes), FALSE if the window cannot satisfy the
 * requirement.
 *
 * @remarks
 ...
 * last-chance vote - this is how a device is re-assigned its own
 * boot configuration.
 */
CODE_SEG("PAGE")
BOOLEAN
    ...
    _Inout_ PARBITER_ALLOCATION_STATE ArbState)
{
    ...
    /* A zero-length requirement is trivially satisfied at the window base. */
     ...
     * than letting the top-down search pick an untranslatable one (see
     * ArbpReuseOwnedInterrupt).  No-op for every other resource type.
     */
    ...
    if (!NT_SUCCESS(Status))
    {
         ...
         * This mechanism is now windows "internally allows this".
         */
 ...
 * physical device object, which RtlDeleteOwnersRanges,
 * OverrideConflict and backtracking all match on.
 *
 * @remarks
 * ADD_IF_CONFLICT is required because override-allowed solutions
 ...
 * being withdrawn, matched by the entry's physical device object.
 */
CODE_SEG("PAGE")
VOID
NTAPI
