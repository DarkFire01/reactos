    ...
    IN ULONG ClusterSize)
{
    ...
    // Lock the volume and dismount any filesystem mounted on it BEFORE writing:
    // formatting must reach the raw partition, including its very last sector
    // (the backup boot sector), and must not be served by a live filesystem's
    // view of the old volume. The dismount in the cleanup path below then
    // forces a remount of the freshly written filesystem.
