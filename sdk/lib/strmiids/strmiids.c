...
/* Added for the wine-10.1-dev DirectShow resync: strmbase mediatype.c
   carries a debug name table that references every GUID in uuids.h and
   ksuuids.h, including these, which this hand-written list never had.
   Values taken from sdk/include/psdk/ksuuids.h where present, otherwise
   from upstream uuids.h. */
...
/* The wine-10.1-dev resync of dxsdk/uuids.h brought in 159 entries this
   hand-written list never had. strmbase mediatype.c carries a debug name
   table that references every entry in uuids.h and ksuuids.h, so each one
   declared there has to be defined somewhere too. Generated from those two
   headers; regenerate the same way after any future resync. */
