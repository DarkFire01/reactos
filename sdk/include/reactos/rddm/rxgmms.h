...
/* VIDMM_ALLOCATION_INFO::Flags */
#define VIDMM_ALLOC_F_CPU_VISIBLE 0x00000001
#define VIDMM_ALLOC_F_PRIMARY     0x00000002

/** @brief Placement request for VidMmCreateAllocation. Zero Size or Alignment means one page. */
typedef struct _VIDMM_ALLOCATION_INFO
{
    SIZE_T Size;
    ULONG  Alignment;
    ULONG  Flags;
    ULONG  SupportedSegmentSet; /* bit (Id - 1) per segment, 0 = any */
} VIDMM_ALLOCATION_INFO, *PVIDMM_ALLOCATION_INFO;

/** @brief Place an allocation in a segment; returns an opaque handle (NULL on fail). */
PVOID
NTAPI
...
/** @brief An allocation's offset within its segment, and the segment id (0 if never placed). */
ULONGLONG
NTAPI
...
/** @brief Map an allocation for CPU access. Release with VidMmFreeCpuVisibleAddress. */
PVOID
NTAPI
