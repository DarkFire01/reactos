 ...
 * PURPOSE:     LPC protocol shared by the POSIX client (PSXDLL) and server (PSXSS)
...
// C_ASSERT) to be included first, e.g. <ndk/lpctypes.h>.
//
...
//
// All subsystem objects live under \PSXSS. Per-session names take the session id.
//
...
//
// Bulk payloads (buffers, paths, struct stat, exec argv) go through a section mapped
// by both sides. Messages carry server-relative pointers into it.
//
...
//
// Bulk transfer sub-ops carried in DataChannel for the data phase of read/write.
// Data moves through the shared section one page at a time.
//
...
//
// Index into the PSXSS dispatch table, taken from the ApiNumber field of the message.
// Extension opcodes live in psxext.h.
//
typedef enum _PSX_API_NUMBER
{
    ...
    PsxApiLseek         = 0x2E,
    PsxApiTcgetattr     = 0x2F, // tty control family 0x2F-0x36 (served by posix.exe)
    ...
    PsxApiMaxApiNumber  = 0x3F  // dispatch table size
} PSX_API_NUMBER;
...
//
// Connection kind passed in the NtConnectPort connection data
//
...
//
// These overlay the message Data area at offset 0x30. Pointer fields (Path.Buffer,
// StatBuffer) are server-relative addresses into the shared section.
//
...
//
// Set in a reply interrupted by a signal; the client resends when Errno is EINTR.
//
#define PSX_RETRY_TAG       15

//
// POSIX API message: the 24-byte PORT_MESSAGE header followed by the POSIX body.
// Field offsets are part of the protocol and asserted below.
//
typedef struct _PSX_API_MESSAGE
{
    ...
    ULONG        RetryTag;      // 0x2C: PSX_RETRY_TAG when the call must be resent
