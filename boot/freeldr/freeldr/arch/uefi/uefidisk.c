    ...
    OUT PULONG BootPartition)
{
    ...
    PartitionNum = (OffsetToBoot - UefiBootRootIdentifier); /* Partitons start with 1 in ARC, but; UEFIs root drive identitfier is also first partition*/
    if (PartitionNum == 0)
    {
        ERR("Boot PartitionNumber is 0\n");
    }
...
UefiSetupBlockDevices()
{
    ...
    handles = MmAllocateMemoryWithType((handle_size + (EFI_PAGE_SIZE * 2) * 2), LoaderFirmwareTemporary);
    Status = GlobalSystemTable->BootServices->LocateHandle(ByProtocol, &bioGuid, NULL, &handle_size, handles);
    ...
    for (i = 0; i < SystemHandleCount; ++i)
    {
        ...
        {
            OffsetToBoot = i; /* The Drive offset in a list of handles */
        }
		
        if (EFI_ERROR(Status) || bio == NULL || bio->Media->BlockSize==0 || bio->Media->BlockSize > 2048){
        ...
        else if(handles[i] == PublicBootHandle)
        {
            ...
            ULONG increment = 0;
            ULONG i;
            /* Grab the offset into the array of handles and decrement per volume (valid partition)*/
...
UefiDiskGetDriveGeometry(UCHAR DriveNumber, PGEOMETRY Geometry)
{
    ...
    UefiDriveNumber =  InternalUefiDisk[DriveNumber - FIRST_BIOS_DISK].UefiRootNumber;
    GlobalSystemTable->BootServices->HandleProtocol(handles[UefiDriveNumber], &bioGuid, (void **) &bio);
    Geometry->Cylinders = 1;      // Isn't relevent for the UEFI BIO protocol
    Geometry->Heads = 1;          // Isn't relevent for the UEFI BIO protocol
