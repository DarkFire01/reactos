...
{
    //TODO: maybe figure out a better way to apply this calculation, UEFI sees the partition 0 as the first partition of the disk.
    printf("UefiGetBootPartitionEntry: DriveNumber: %d\n", DriveNumber - FIRST_BIOS_DISK);
       *BootPartition = (OffsetToBoot - UefiBootRootIdentifier - FIRST_PARTITION);
       ULONG PartitionNum = *BootPartition;
       if (PartitionNum == 0)
       {
    ...
    *BootPartition = PartitionNum;
    printf("UefiGetBootPartitionEntry: Boot Partition is: %d\n", PartitionNum);
    /*  */
    return TRUE;
}
