 ...
 * PURPOSE:     Direct3D 10 visual: a per-pixel lit octahedron
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d10vis_model");

    hwnd = vis_create_window("Direct3D 10: lit octahedron");
    if (!d10_open(&s, hwnd)) { skip_("no Direct3D 10 device"); goto done; }
    ok_(1, "created a Direct3D 10 device and swap chain");

    if (!(vsb = d10_compile(hlsl_vs, "vs_4_0")) || !(psb = d10_compile(hlsl_ps_lit, "ps_4_0")))
    {
    ...
    d10_fill(verts, vis_model, VIS_MODEL_VERTICES);
    vb = d10_make_buffer(s.device, verts, sizeof(verts), D3D10_BIND_VERTEX_BUFFER);
    ib = d10_make_buffer(s.device, vis_model_indices, sizeof(vis_model_indices), D3D10_BIND_INDEX_BUFFER);
    ...
    vis_lookat(&view, 0.0f, 1.2f, -5.5f, 0.0f, 0.0f, 0.0f);
    ...
    if (n) vis_check_rendered(sample, n, 0xff211710);
