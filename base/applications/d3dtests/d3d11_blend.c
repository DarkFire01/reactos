 ...
 * PURPOSE:     Direct3D 11: alpha blending verified by reading the result back
 */
...
static const char ps_source[] =
    "float4 main() : SV_TARGET { return float4(1.0f, 1.0f, 1.0f, 0.5f); }\n";
...
int main(void)
{
    ...
    hr = ID3D11Device_CreateBlendState(device, &bsd, &blend);
    ok_(SUCCEEDED(hr) && blend != NULL, "CreateBlendState returned 0x%08lx", hr);
    ...
    if (!vs || !ps || !layout || !vb || !blend) goto cleanup;

    ID3D11DeviceContext_OMSetRenderTargets(context, 1, &rtv, NULL);
    ID3D11DeviceContext_ClearRenderTargetView(context, rtv, clear);
    ID3D11DeviceContext_OMSetBlendState(context, blend, factor, 0xffffffff);
    ...
    /* White at 50% alpha over black should land near mid grey. */
    ...
    ID3D11DeviceContext_OMSetBlendState(context, NULL, factor, 0xffffffff);

cleanup:
    D3DTEST_RELEASE(staging);
    D3DTEST_RELEASE(rtv);
    D3DTEST_RELEASE(target);
    D3DTEST_RELEASE(blend);
    D3DTEST_RELEASE(vb);
