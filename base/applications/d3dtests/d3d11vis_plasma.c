 ...
 * PURPOSE:     Direct3D 11 visual: an animated plasma from a full-screen pixel shader
 */
...
/* Everything is done in the pixel shader; light.x is the animation time, which
   keeps struct d11_constants identical to the one the other scenes use. */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d11vis_plasma");

    hwnd = vis_create_window("Direct3D 11: pixel-shader plasma");
    ...
    if (!(vsb = d11_compile(hlsl_vs, "vs_4_0")) || !(psb = d11_compile(hlsl_ps_plasma, "ps_4_0")))
    {
