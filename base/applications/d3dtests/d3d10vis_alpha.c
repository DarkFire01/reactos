 ...
 * PURPOSE:     Direct3D 10 visual: a translucent cube blended over a solid one
 */
...
/* The blended pass needs a per-draw opacity. light.w is spare in the constant
   ...
   struct d10_constants stays exactly as the siblings have it. */
...
int main(int argc, char **argv)
{
    ...
    ID3D10BlendState *opaque = NULL, *blended = NULL;
    ...
    test_begin("d3d10vis_alpha");

    hwnd = vis_create_window("Direct3D 10: alpha blending");
    if (!d10_open(&s, hwnd)) { skip_("no Direct3D 10 device"); goto done; }
    ok_(1, "created a Direct3D 10 device and swap chain");

    if (!(vsb = d10_compile(hlsl_vs, "vs_4_0")) || !(psb = d10_compile(hlsl_ps_alpha, "ps_4_0")))
    {
    ...
    /* D3D10 has one blend equation for all eight targets, only the enable and
       the write mask are per target. */
    memset(&bsd, 0, sizeof(bsd));
    for (i = 0; i < 8; i++)
    {
        bsd.BlendEnable[i] = FALSE;
        bsd.RenderTargetWriteMask[i] = D3D10_COLOR_WRITE_ENABLE_ALL;
    }
    bsd.SrcBlend = D3D10_BLEND_ONE;
    bsd.DestBlend = D3D10_BLEND_ZERO;
    ...
    ID3D10Device_CreateBlendState(s.device, &bsd, &opaque);

    bsd.BlendEnable[0] = TRUE;
    bsd.SrcBlend = D3D10_BLEND_SRC_ALPHA;
    bsd.DestBlend = D3D10_BLEND_INV_SRC_ALPHA;
    ID3D10Device_CreateBlendState(s.device, &bsd, &blended);
    ...
    while (vis_frame(frame++))
    {
        ...
        ID3D10Device_OMSetBlendState(s.device, opaque, factor, 0xffffffff);
        ...
        memset(&constants, 0, sizeof(constants));
        d10_store_matrix(constants.wvp, &wvp);
        d10_store_matrix(constants.world, &ghost);
        constants.light[0] = -0.4f;
        constants.light[1] = 0.7f;
        constants.light[2] = -0.6f;
        ...
        ID3D10Device_OMSetBlendState(s.device, blended, factor, 0xffffffff);
        ID3D10Device_DrawIndexed(s.device, VIS_CUBE_INDICES, 0, 0);
