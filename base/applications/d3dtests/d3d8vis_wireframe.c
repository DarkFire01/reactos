 ...
 * PURPOSE:     Direct3D 8 visual: the model in wireframe fill mode
 */
...
static D3DTEST_UNUSED int d8_sample(struct d8_scene *s, DWORD *out, int max)
{
    ...
    /* Wireframe edges are a pixel wide, so the scattered grid the other tests
       use walks straight past them. Read one solid run of pixels across the
       middle of the frame instead: any closed outline centred in view has to
       cross it. */
    {
        int y = (int)desc.Height / 2;
        int x0 = ((int)desc.Width - max) / 2;
        const BYTE *row = (const BYTE *)lr.pBits + y * lr.Pitch;

        if (x0 < 0)
            x0 = 0;
        for (i = 0; i < max && x0 + i < (int)desc.Width; i++)
            out[n++] = *(const DWORD *)(row + (x0 + i) * 4);
    }
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d8vis_wireframe");

    hwnd = vis_create_window("Direct3D 8: wireframe");
    if (!d8_open(&s, hwnd)) { skip_("no Direct3D 8 device"); goto done; }

    d8_fill_colour(verts, vis_model, VIS_MODEL_VERTICES);
    d8_camera(&s, 5.5f);

    hr = IDirect3DDevice8_SetRenderState(s.device, D3DRS_FILLMODE, D3DFILL_WIREFRAME);
    ok_(SUCCEEDED(hr), "SetRenderState(FILLMODE, WIREFRAME) returned 0x%08lx", hr);
    ...
    ok_(frame > 1, "rendered %d wireframe frames", frame - 1);

    n = d8_sample(&s, sample, ARRAYSIZE(sample));
    if (n) vis_check_rendered(sample, n, 0xff000000);

    IDirect3DDevice8_SetRenderState(s.device, D3DRS_FILLMODE, D3DFILL_SOLID);
    vis_wait_if_held();
done:
    d8_close(&s);
