 ...
 * PURPOSE:     Direct3D 11 visual: a model rendered to a texture, then mapped onto a cube
 */
...
int main(int argc, char **argv)
{
    ...
    ID3D11PixelShader *ps_tex = NULL, *ps_lit = NULL;
    ID3D11Buffer *vb = NULL, *ib = NULL, *mvb = NULL, *mib = NULL, *cb = NULL;
    ID3D11Texture2D *offtex = NULL, *offdepth = NULL;
    ID3D11RenderTargetView *offrtv = NULL;
    ID3D11DepthStencilView *offdsv = NULL;
    ID3D11ShaderResourceView *srv = NULL, *nosrv = NULL;
    ID3D11SamplerState *sampler = NULL;
    ID3D10Blob *vsb = NULL, *psb_tex = NULL, *psb_lit = NULL;
    struct d11_vertex cubeverts[VIS_CUBE_VERTICES];
    struct d11_vertex modelverts[VIS_MODEL_VERTICES];
    ...
    test_begin("d3d11vis_rtt");

    hwnd = vis_create_window("Direct3D 11: render to texture");
    ...
    if (!(vsb = d11_compile(hlsl_vs, "vs_4_0"))
        || !(psb_tex = d11_compile(hlsl_ps_texture, "ps_4_0"))
        || !(psb_lit = d11_compile(hlsl_ps_lit, "ps_4_0")))
    {
    ...
    ID3D11Device_CreatePixelShader(s.device, ID3D10Blob_GetBufferPointer(psb_tex),
            ID3D10Blob_GetBufferSize(psb_tex), NULL, &ps_tex);
    ID3D11Device_CreatePixelShader(s.device, ID3D10Blob_GetBufferPointer(psb_lit),
            ID3D10Blob_GetBufferSize(psb_lit), NULL, &ps_lit);
    ...
    d11_fill(cubeverts, vis_cube, VIS_CUBE_VERTICES);
    d11_fill(modelverts, vis_model, VIS_MODEL_VERTICES);
    vb = d11_make_vb(s.device, cubeverts, sizeof(cubeverts));
    ib = d11_make_ib(s.device, vis_cube_indices, sizeof(vis_cube_indices));
    mvb = d11_make_vb(s.device, modelverts, sizeof(modelverts));
    mib = d11_make_ib(s.device, vis_model_indices, sizeof(vis_model_indices));
    ...
    /* The offscreen target: drawn into, then read as a texture. */
    memset(&td, 0, sizeof(td));
    ...
    if (FAILED(ID3D11Device_CreateTexture2D(s.device, &td, NULL, &offtex)))
    {
        skip_("this device would not give us a render target that is also a texture");
        goto done;
    }
    ID3D11Device_CreateRenderTargetView(s.device, (ID3D11Resource *)offtex, NULL, &offrtv);
    ID3D11Device_CreateShaderResourceView(s.device, (ID3D11Resource *)offtex, NULL, &srv);
    ...
    td.Usage = D3D11_USAGE_DEFAULT;
    td.BindFlags = D3D11_BIND_DEPTH_STENCIL;
    if (SUCCEEDED(ID3D11Device_CreateTexture2D(s.device, &td, NULL, &offdepth)))
        ID3D11Device_CreateDepthStencilView(s.device, (ID3D11Resource *)offdepth, NULL, &offdsv);
    ...
    while (vis_frame(frame++))
    {
        ...
        ID3D11DeviceContext_PSSetShaderResources(s.context, 0, 1, &nosrv);
        ID3D11DeviceContext_OMSetRenderTargets(s.context, 1, &offrtv, offdsv);
        ID3D11DeviceContext_ClearRenderTargetView(s.context, offrtv, offclear);
        if (offdsv)
            ID3D11DeviceContext_ClearDepthStencilView(s.context, offdsv,
        ...
        ID3D11DeviceContext_IASetVertexBuffers(s.context, 0, 1, &mvb, &stride, &offset);
        ID3D11DeviceContext_IASetIndexBuffer(s.context, mib, DXGI_FORMAT_R16_UINT, 0);
        ...
        ID3D11DeviceContext_PSSetShader(s.context, ps_lit, NULL, 0);
        ...
        ID3D11DeviceContext_PSSetShader(s.context, ps_tex, NULL, 0);
        ID3D11DeviceContext_PSSetShaderResources(s.context, 0, 1, &srv);
        ID3D11DeviceContext_PSSetSamplers(s.context, 0, 1, &sampler);
        ID3D11DeviceContext_DrawIndexed(s.context, VIS_CUBE_INDICES, 0, 0);
    ...
    /* Prove the offscreen pass actually drew into the texture. Sampling the
       ...
       check does not need. */
    {
        ID3D11Texture2D *offstage = NULL;
        D3D11_MAPPED_SUBRESOURCE om;
        D3D11_TEXTURE2D_DESC ostd;
        ...
        ostd.Usage = D3D11_USAGE_STAGING;
        ostd.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
        if (SUCCEEDED(ID3D11Device_CreateTexture2D(s.device, &ostd, NULL, &offstage)))
        {
            ID3D11DeviceContext_CopyResource(s.context, (ID3D11Resource *)offstage,
                                             (ID3D11Resource *)offtex);
            memset(&om, 0, sizeof(om));
            if (SUCCEEDED(ID3D11DeviceContext_Map(s.context, (ID3D11Resource *)offstage,
                                                  0, D3D11_MAP_READ, 0, &om)))
            {
                DWORD column[256];
                int i, distinct;
