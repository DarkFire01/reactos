 ...
 * PURPOSE:     Direct3D 11 visual: two interpenetrating objects sorted by the depth buffer
 */
...
int main(int argc, char **argv)
{
    ...
    DWORD sample[256];
    struct d11_scene s;
    struct vis_mat world, view, proj, wv, wvp, ry, rx, tr;
    D3D11_DEPTH_STENCIL_DESC dsd;
    float clear[4] = { 0.09f, 0.11f, 0.15f, 1.0f };
    UINT stride = sizeof(struct d11_vertex), offset = 0;
    int frame = 0, n;
    HWND hwnd;

    vis_parse_args(argc, argv);
    test_begin("d3d11vis_depth");

    hwnd = vis_create_window("Direct3D 11: depth-buffered intersection");
    ...
    if (!s.dsv)
    {
        skip_("this device gave us no depth buffer, so there is nothing to show");
        goto done;
    }

    if (!(vsb = d11_compile(hlsl_vs, "vs_4_0")) || !(psb = d11_compile(hlsl_ps_lit, "ps_4_0")))
    {
    ...
    if (!vs || !ps || !layout || !vb || !ib || !mvb || !mib || !cb)
    {
        skip_("could not build the scene resources");
        goto done;
    }

    /* Spelled out rather than left to the default state, because the point of
       this test is that the depth comparison is the thing sorting the two
       objects out where they pass through one another. */
    ...
    ID3D11Device_CreateDepthStencilState(s.device, &dsd, &dss);
    ok_(dss != NULL, "created a depth-stencil state with LESS and full depth writes");
    if (!dss) { skip_("no depth-stencil state"); goto done; }

    vis_perspective(&proj, 1.05f, (float)VIS_WIDTH / VIS_HEIGHT, 1.0f, 100.0f);
    vis_lookat(&view, 0.0f, 1.2f, -6.5f, 0.0f, 0.0f, 0.0f);
    ...
    while (vis_frame(frame++))
    {
        ...
        d11_begin(&s, clear);
        ID3D11DeviceContext_OMSetDepthStencilState(s.context, dss, 0);
        ...
        /* The cube, pushed left far enough that its right-hand side is inside
           the octahedron drawn after it. */
        vis_rotate_y(&ry, t);
        vis_rotate_x(&rx, t * 0.5f);
        vis_mul(&world, &rx, &ry);
        vis_translate(&tr, -0.7f, 0.0f, 0.0f);
        ...
        /* The octahedron, counter-rotating and pushed right. Drawn second, so
           only the depth test can keep the near half of the cube in front. */
        vis_rotate_y(&ry, -t * 1.3f);
        vis_translate(&tr, 0.7f, 0.0f, 0.0f);
        vis_mul(&world, &ry, &tr);
        vis_mul(&wv, &world, &view);
        vis_mul(&wvp, &wv, &proj);
    ...
    if (n) vis_check_rendered(sample, n, 0xff261c17);
