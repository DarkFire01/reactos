 ...
 * PURPOSE:     Direct3D 11 visual: a compute shader painting into a UAV, shown on a quad
 */
...
/* The compute shader fills a 256x256 image. It reads the time out of light.x
   so struct d11_constants can stay exactly as the other scenes have it. */
static const char hlsl_cs_plasma[] =
    "cbuffer constants : register(b0)\n"
    "{\n"
    "    float4x4 wvp;\n"
    "    float4x4 world;\n"
    ...
    "    float t = light.x;\n"
    "    float x = tid.x * 0.05f;\n"
    "    float y = tid.y * 0.045f;\n"
    "    float v = sin(x + t) + sin(y - t) + sin((x + y) * 0.7f + t * 0.7f);\n"
    "    float4 c;\n"
    "    c.r = 0.5f + 0.45f * sin(v * 1.4f);\n"
    "    c.g = 0.5f + 0.45f * sin(v * 1.4f + 2.0f);\n"
    "    c.b = 0.5f + 0.45f * sin(v * 1.4f + 4.0f);\n"
    "    c.a = 1.0f;\n"
    "    image[tid.xy] = c;\n"
    "}\n";

static const WORD quad_indices[6] = { 0, 1, 2, 0, 2, 3 };
...
int main(int argc, char **argv)
{
    ...
    ID3D11Texture2D *image = NULL;
    ID3D11UnorderedAccessView *uav = NULL, *nouav = NULL;
    ID3D11ShaderResourceView *srv = NULL, *nosrv = NULL;
    ID3D11SamplerState *sampler = NULL;
    ID3D10Blob *vsb = NULL, *psb = NULL, *csb = NULL;
    struct d11_vertex quad[4];
    struct d11_constants constants;
    DWORD sample[256];
    struct d11_scene s;
    struct vis_mat identity;
    D3D11_UNORDERED_ACCESS_VIEW_DESC uavd;
    D3D11_TEXTURE2D_DESC td;
    float clear[4] = { 0.05f, 0.07f, 0.11f, 1.0f };
    UINT stride = sizeof(struct d11_vertex), offset = 0;
    int frame = 0, n, i;
    HWND hwnd;

    vis_parse_args(argc, argv);
    test_begin("d3d11vis_compute");

    hwnd = vis_create_window("Direct3D 11: compute-shader plasma");
    ...
    if (s.level < D3D_FEATURE_LEVEL_11_0)
    {
        skip_("feature level 0x%04x is below 11_0, so there are no compute shaders",
              (unsigned)s.level);
        goto done;
    }
    ...
    if (!(csb = d11_compile(hlsl_cs_plasma, "cs_5_0")))
    {
        skip_("could not compile the cs_5_0 compute shader");
        goto done;
    }
    ok_(1, "compiled the vertex, pixel and compute shaders");
    ...
    ID3D11Device_CreateComputeShader(s.device, ID3D10Blob_GetBufferPointer(csb),
    ...
    if (!cs)
    {
        skip_("CreateComputeShader failed, so this device cannot run the shader");
        goto done;
    }

    /* A quad already in clip space: the shared vertex shader multiplies by wvp,
       so wvp is left as the identity and these coordinates come out unchanged. */
    memset(quad, 0, sizeof(quad));
    quad[0].pos[0] = -1.0f; quad[0].pos[1] =  1.0f; quad[0].uv[0] = 0.0f; quad[0].uv[1] = 0.0f;
    quad[1].pos[0] =  1.0f; quad[1].pos[1] =  1.0f; quad[1].uv[0] = 1.0f; quad[1].uv[1] = 0.0f;
    quad[2].pos[0] =  1.0f; quad[2].pos[1] = -1.0f; quad[2].uv[0] = 1.0f; quad[2].uv[1] = 1.0f;
    quad[3].pos[0] = -1.0f; quad[3].pos[1] = -1.0f; quad[3].uv[0] = 0.0f; quad[3].uv[1] = 1.0f;
    for (i = 0; i < 4; i++)
    {
        quad[i].normal[2] = -1.0f;
        quad[i].colour[0] = quad[i].colour[1] = quad[i].colour[2] = quad[i].colour[3] = 1.0f;
    }

    vb = d11_make_vb(s.device, quad, sizeof(quad));
    ib = d11_make_ib(s.device, quad_indices, sizeof(quad_indices));
    ...
    if (FAILED(ID3D11Device_CreateTexture2D(s.device, &td, NULL, &image)))
    {
        skip_("this device will not write R8G8B8A8 through an unordered access view");
        goto done;
    }
    ...
    ID3D11Device_CreateUnorderedAccessView(s.device, (ID3D11Resource *)image, &uavd, &uav);
    ID3D11Device_CreateShaderResourceView(s.device, (ID3D11Resource *)image, NULL, &srv);
    if (!uav || !srv)
    {
        skip_("could not view the image as both an unordered access view and a texture");
        goto done;
    }
    ok_(1, "created a 256x256 UAV the compute shader writes and the pixel shader reads");
    ...
    while (vis_frame(frame++))
    {
        ...
        d11_store_matrix(constants.wvp, &identity);
        d11_store_matrix(constants.world, &identity);
        constants.light[0] = t;
        d11_update_cb(s.context, cb, &constants, sizeof(constants));

        /* Generate the image. The texture cannot be bound to the pixel stage
           while the compute stage writes it, so drop it there first. */
        ID3D11DeviceContext_PSSetShaderResources(s.context, 0, 1, &nosrv);
        ID3D11DeviceContext_CSSetShader(s.context, cs, NULL, 0);
        ID3D11DeviceContext_CSSetConstantBuffers(s.context, 0, 1, &cb);
        ID3D11DeviceContext_CSSetUnorderedAccessViews(s.context, 0, 1, &uav, NULL);
        ID3D11DeviceContext_Dispatch(s.context, 32, 32, 1);
        ID3D11DeviceContext_CSSetUnorderedAccessViews(s.context, 0, 1, &nouav, NULL);
