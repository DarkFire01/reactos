 ...
 * PURPOSE:     Direct3D 10 visual: a shaded rotating cube
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d10vis_cube");

    hwnd = vis_create_window("Direct3D 10: rotating cube");
    if (!d10_open(&s, hwnd)) { skip_("no Direct3D 10 device"); goto done; }
    ok_(1, "created a Direct3D 10 device and swap chain");

    if (!(vsb = d10_compile(hlsl_vs, "vs_4_0")) || !(psb = d10_compile(hlsl_ps_colour, "ps_4_0")))
    {
