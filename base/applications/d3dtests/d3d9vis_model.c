 ...
 * PURPOSE:     Direct3D 9 visual: a lit octahedron model
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d9vis_model");

    hwnd = vis_create_window("Direct3D 9: lit octahedron");
    ...
    material.Diffuse.r = 0.85f; material.Diffuse.g = 0.7f; material.Diffuse.b = 0.4f;
    material.Diffuse.a = 1.0f;
    material.Ambient.r = material.Ambient.g = material.Ambient.b = 0.25f;
    ...
    light.Direction.x = -0.4f; light.Direction.y = -0.7f; light.Direction.z = 0.6f;
    hr = IDirect3DDevice9_SetLight(s.device, 0, &light);
    ok_(SUCCEEDED(hr), "SetLight returned 0x%08lx", hr);
    IDirect3DDevice9_LightEnable(s.device, 0, TRUE);

    IDirect3DDevice9_SetRenderState(s.device, D3DRS_LIGHTING, TRUE);
    IDirect3DDevice9_SetRenderState(s.device, D3DRS_AMBIENT, 0xff303030);
