 ...
 * PURPOSE:     Direct3D 7 visual: a lit octahedron model
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d7vis_model");

    hwnd = vis_create_window("Direct3D 7: lit octahedron");
    if (!d7_open(&s, hwnd))
    {
    ...
    material.dcvDiffuse.r = 0.9f;
    material.dcvDiffuse.g = 0.75f;
    material.dcvDiffuse.b = 0.4f;
    material.dcvDiffuse.a = 1.0f;
    material.dcvAmbient.r = 0.2f;
    material.dcvAmbient.g = 0.2f;
    material.dcvAmbient.b = 0.25f;
    hr = IDirect3DDevice7_SetMaterial(s.device, &material);
    ok_(SUCCEEDED(hr), "SetMaterial returned 0x%08lx", hr);

    memset(&light, 0, sizeof(light));
    light.dltType = D3DLIGHT_DIRECTIONAL;
    light.dcvDiffuse.r = light.dcvDiffuse.g = light.dcvDiffuse.b = light.dcvDiffuse.a = 1.0f;
    light.dvDirection.x = -0.4f;
    light.dvDirection.y = -0.7f;
    light.dvDirection.z = 0.6f;
    hr = IDirect3DDevice7_SetLight(s.device, 0, &light);
    ok_(SUCCEEDED(hr), "SetLight returned 0x%08lx", hr);
    IDirect3DDevice7_LightEnable(s.device, 0, TRUE);

    IDirect3DDevice7_SetRenderState(s.device, D3DRENDERSTATE_LIGHTING, TRUE);
    IDirect3DDevice7_SetRenderState(s.device, D3DRENDERSTATE_AMBIENT, 0x20202020);
    ...
    while (vis_frame(frame++))
    {
        ...
        IDirect3DDevice7_Clear(s.device, 0, NULL,
                D3DCLEAR_TARGET | (s.depth ? D3DCLEAR_ZBUFFER : 0), 0x00101820, 1.0f, 0);
    ...
    ok_(frame > 1, "rendered %d frames of the lit model", frame - 1);

    n = d7_sample(&s, sample, ARRAYSIZE(sample));
    if (n)
        vis_check_rendered(sample, n, 0x00101820);
