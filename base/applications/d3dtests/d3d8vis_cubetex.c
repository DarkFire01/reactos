 ...
 * PURPOSE:     Direct3D 8 visual: a textured rotating cube
 */
...
{
    IDirect3DTexture8 *texture = NULL;
    struct d8_vertex verts[VIS_CUBE_VERTICES];
    ...
    IDirect3DDevice8_SetTexture(s.device, 0, (IDirect3DBaseTexture8 *)texture);
    ...
    ok_(frame > 1, "rendered %d textured frames", frame - 1);

    n = d8_sample(&s, sample, ARRAYSIZE(sample));
    if (n) vis_check_rendered(sample, n, 0xff181c28);
