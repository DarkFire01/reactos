 ...
 * PURPOSE:     Direct3D 10 visual: a model rendered to a texture, then mapped onto a cube
 */
...
int main(int argc, char **argv)
{
    ...
    ID3D10PixelShader *ps_tex = NULL, *ps_lit = NULL;
    ID3D10Buffer *vb = NULL, *ib = NULL, *mvb = NULL, *mib = NULL, *cb = NULL;
    ID3D10Texture2D *offtex = NULL, *offdepth = NULL;
    ID3D10RenderTargetView *offrtv = NULL;
    ID3D10DepthStencilView *offdsv = NULL;
    ID3D10ShaderResourceView *srv = NULL, *nosrv = NULL;
    ID3D10SamplerState *sampler = NULL;
    ID3D10Blob *vsb = NULL, *psb_tex = NULL, *psb_lit = NULL;
    struct d10_vertex cubeverts[VIS_CUBE_VERTICES];
    struct d10_vertex modelverts[VIS_MODEL_VERTICES];
    struct d10_constants constants;
    DWORD sample[256];
    struct d10_scene s;
    struct vis_mat world, view, proj, offproj, wv, wvp, ry, rx;
    D3D10_TEXTURE2D_DESC td;
    D3D10_BUFFER_DESC bd;
    D3D10_VIEWPORT vp;
    float clear[4] = { 0.06f, 0.08f, 0.12f, 1.0f };
    float offclear[4] = { 0.35f, 0.10f, 0.45f, 1.0f };
    UINT stride = sizeof(struct d10_vertex), offset = 0;
    int frame = 0, n;
    HWND hwnd;

    vis_parse_args(argc, argv);
    test_begin("d3d10vis_rtt");

    hwnd = vis_create_window("Direct3D 10: render to texture");
    if (!d10_open(&s, hwnd)) { skip_("no Direct3D 10 device"); goto done; }
    ok_(1, "created a Direct3D 10 device and swap chain");

    if (!(vsb = d10_compile(hlsl_vs, "vs_4_0"))
        || !(psb_tex = d10_compile(hlsl_ps_texture, "ps_4_0"))
        || !(psb_lit = d10_compile(hlsl_ps_lit, "ps_4_0")))
    {
    ...
    ID3D10Device_CreatePixelShader(s.device, ID3D10Blob_GetBufferPointer(psb_tex),
            ID3D10Blob_GetBufferSize(psb_tex), &ps_tex);
    ID3D10Device_CreatePixelShader(s.device, ID3D10Blob_GetBufferPointer(psb_lit),
            ID3D10Blob_GetBufferSize(psb_lit), &ps_lit);
    ...
    d10_fill(cubeverts, vis_cube, VIS_CUBE_VERTICES);
    d10_fill(modelverts, vis_model, VIS_MODEL_VERTICES);
    vb = d10_make_buffer(s.device, cubeverts, sizeof(cubeverts), D3D10_BIND_VERTEX_BUFFER);
    ib = d10_make_buffer(s.device, vis_cube_indices, sizeof(vis_cube_indices), D3D10_BIND_INDEX_BUFFER);
    mvb = d10_make_buffer(s.device, modelverts, sizeof(modelverts), D3D10_BIND_VERTEX_BUFFER);
    mib = d10_make_buffer(s.device, vis_model_indices, sizeof(vis_model_indices), D3D10_BIND_INDEX_BUFFER);
    ...
    if (!vs || !ps_tex || !ps_lit || !layout || !vb || !ib || !mvb || !mib || !cb)
    {
        skip_("could not build the scene resources");
        goto done;
    }

    /* The offscreen target: drawn into, then read as a texture. R8G8B8A8 rather
       than B8G8R8A8, which a 10.0 feature level need only support on the swap
       chain and not as a shader resource. */
    memset(&td, 0, sizeof(td));
    ...
    if (FAILED(ID3D10Device_CreateTexture2D(s.device, &td, NULL, &offtex)))
    {
        skip_("this device would not give us a render target that is also a texture");
        goto done;
    }
    ID3D10Device_CreateRenderTargetView(s.device, (ID3D10Resource *)offtex, NULL, &offrtv);
    ID3D10Device_CreateShaderResourceView(s.device, (ID3D10Resource *)offtex, NULL, &srv);
    ok_(offrtv != NULL && srv != NULL,
        "created a render target view and a shader resource view on the same texture");
    if (!offrtv || !srv) { skip_("could not view the offscreen texture both ways"); goto done; }
    ...
    td.Usage = D3D10_USAGE_DEFAULT;
    td.BindFlags = D3D10_BIND_DEPTH_STENCIL;
    if (SUCCEEDED(ID3D10Device_CreateTexture2D(s.device, &td, NULL, &offdepth)))
        ID3D10Device_CreateDepthStencilView(s.device, (ID3D10Resource *)offdepth, NULL, &offdsv);
    ...
    if (!sampler) { skip_("could not create a sampler state"); goto done; }

    vis_perspective(&proj, 1.05f, (float)VIS_WIDTH / VIS_HEIGHT, 1.0f, 100.0f);
    vis_perspective(&offproj, 1.05f, 1.0f, 1.0f, 100.0f);
    ...
    while (vis_frame(frame++))
    {
        ...
        /* Pass one: the octahedron into the offscreen texture. Unbind it from
           the pixel stage first or it cannot be a render target this frame. */
        ID3D10Device_PSSetShaderResources(s.device, 0, 1, &nosrv);
        ID3D10Device_OMSetRenderTargets(s.device, 1, &offrtv, offdsv);
        ID3D10Device_ClearRenderTargetView(s.device, offrtv, offclear);
        if (offdsv)
            ID3D10Device_ClearDepthStencilView(s.device, offdsv, D3D10_CLEAR_DEPTH, 1.0f, 0);
        memset(&vp, 0, sizeof(vp));
        vp.Width = 256;
        vp.Height = 256;
        vp.MaxDepth = 1.0f;
        ...
        vis_rotate_y(&ry, t * 1.7f);
        vis_rotate_x(&rx, t * 0.9f);
        vis_mul(&world, &rx, &ry);
        vis_mul(&wv, &world, &view);
        vis_mul(&wvp, &wv, &offproj);
        ...
        ID3D10Device_IASetVertexBuffers(s.device, 0, 1, &mvb, &stride, &offset);
        ID3D10Device_IASetIndexBuffer(s.device, mib, DXGI_FORMAT_R16_UINT, 0);
        ...
        ID3D10Device_PSSetShader(s.device, ps_lit);
        ID3D10Device_PSSetConstantBuffers(s.device, 0, 1, &cb);
        ID3D10Device_DrawIndexed(s.device, VIS_MODEL_INDICES, 0, 0);

        /* Pass two: that texture wrapped round the cube, on screen. */
        vis_rotate_y(&ry, t * 0.6f);
        vis_rotate_x(&rx, t * 0.3f);
        vis_mul(&world, &rx, &ry);
        vis_mul(&wv, &world, &view);
        vis_mul(&wvp, &wv, &proj);
        memset(&constants, 0, sizeof(constants));
        ...
        ID3D10Device_PSSetShader(s.device, ps_tex);
    ...
    /* Prove the offscreen pass actually drew into the texture. Sampling the
       cube on screen cannot tell a working render to texture from one that
       handed back a uniformly cleared surface -- either way the cube is
       covered in *something*. So read the target itself: if the octahedron
       landed in it, its centre column holds more than the one clear colour.
       Comparing against the clear colour directly would mean hand-rounding
       the float clear to bytes, which is exactly the sort of off-by-one this
       check does not need. */
    {
        ID3D10Texture2D *offstage = NULL;
        D3D10_MAPPED_TEXTURE2D om;
        D3D10_TEXTURE2D_DESC ostd;

        memset(&ostd, 0, sizeof(ostd));
        ostd.Width = ostd.Height = 256;
        ostd.MipLevels = ostd.ArraySize = 1;
        ostd.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
        ostd.SampleDesc.Count = 1;
        ostd.Usage = D3D10_USAGE_STAGING;
        ostd.CPUAccessFlags = D3D10_CPU_ACCESS_READ;
        if (SUCCEEDED(ID3D10Device_CreateTexture2D(s.device, &ostd, NULL, &offstage)))
        {
            ID3D10Device_CopyResource(s.device, (ID3D10Resource *)offstage,
                                      (ID3D10Resource *)offtex);
            memset(&om, 0, sizeof(om));
            if (SUCCEEDED(ID3D10Texture2D_Map(offstage, 0, D3D10_MAP_READ, 0, &om)))
            {
                DWORD column[256];
                int i, distinct;

                for (i = 0; i < 256; i++)
                    column[i] = *(DWORD *)((BYTE *)om.pData + i * om.RowPitch + 128 * 4);
                ID3D10Texture2D_Unmap(offstage, 0);
                distinct = vis_count_distinct(column, 256, 0x00ffffff);
                ok_(distinct >= 2,
                    "the offscreen target holds %d distinct colour(s) down its centre column",
                    distinct);
            }
            ID3D10Texture2D_Release(offstage);
        }
    }

    n = d10_sample(&s, sample, ARRAYSIZE(sample));
    if (n) vis_check_rendered(sample, n, 0xff1f140f);
    ...
    D3DTEST_RELEASE(srv);
    D3DTEST_RELEASE(offdsv);
    D3DTEST_RELEASE(offdepth);
    D3DTEST_RELEASE(offrtv);
    D3DTEST_RELEASE(offtex);
    D3DTEST_RELEASE(cb);
    D3DTEST_RELEASE(mib);
    D3DTEST_RELEASE(mvb);
    D3DTEST_RELEASE(ib);
    ...
    D3DTEST_RELEASE(vs);
    D3DTEST_RELEASE(ps_tex);
    D3DTEST_RELEASE(ps_lit);
    D3DTEST_RELEASE(vsb);
    D3DTEST_RELEASE(psb_tex);
    D3DTEST_RELEASE(psb_lit);
    d10_close(&s);
