 ...
 * PURPOSE:     Direct3D 8 visual: a cube receding into linear fog
 */
...
static D3DTEST_UNUSED int d8_sample(struct d8_scene *s, DWORD *out, int max)
{
    ...
    /* The cube swings far enough away to cover well under a percent of the
       frame, and the scattered grid the other tests use is a fixed pattern, so
       it misses it every time rather than just sometimes. Read a solid run of
       pixels instead. It has to be a column, not a row: the camera sits above
       the origin and looks down at it, so the cube climbs towards the horizon
       as it recedes and leaves the middle row, but it never moves sideways. */
    {
        int x = (int)desc.Width / 2;
        int y0 = ((int)desc.Height - max) / 2;

        if (y0 < 0)
            y0 = 0;
        for (i = 0; i < max && y0 + i < (int)desc.Height; i++)
            out[n++] = *(const DWORD *)((const BYTE *)lr.pBits
                                        + (y0 + i) * lr.Pitch + x * 4);
    }
...
int main(int argc, char **argv)
{
    ...
    float fog_start = 4.0f, fog_end = 18.0f;
    D3DCAPS8 caps;
    int frame = 0, n;
    HWND hwnd;
    HRESULT hr;
    ...
    test_begin("d3d8vis_fog");

    hwnd = vis_create_window("Direct3D 8: linear fog");
    ...
    hr = IDirect3DDevice8_SetRenderState(s.device, D3DRS_FOGENABLE, TRUE);
    ok_(SUCCEEDED(hr), "SetRenderState(FOGENABLE) returned 0x%08lx", hr);
    IDirect3DDevice8_SetRenderState(s.device, D3DRS_FOGCOLOR, 0xff405060);
    IDirect3DDevice8_SetRenderState(s.device, D3DRS_FOGVERTEXMODE, D3DFOG_LINEAR);
    IDirect3DDevice8_SetRenderState(s.device, D3DRS_FOGSTART, *(DWORD *)&fog_start);
    IDirect3DDevice8_SetRenderState(s.device, D3DRS_FOGEND, *(DWORD *)&fog_end);
    ...
    IDirect3DDevice8_SetRenderState(s.device, D3DRS_FOGENABLE, FALSE);
    vis_wait_if_held();
done:
    d8_close(&s);
