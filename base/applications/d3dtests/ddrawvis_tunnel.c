 ...
 * PURPOSE:     DirectDraw visual: a per-pixel texture-mapped tunnel effect
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("ddrawvis_tunnel");

    hwnd = vis_create_window("DirectDraw: tunnel");
    if (!dd_open(&s, hwnd))
    {
    ...
    vis_tex_checker(tex, 128, 128, 16, 0xff2060c0, 0xfff0e0a0);
    ok_(1, "built the 128x128 tunnel texture");
    ...
    while (vis_frame(frame++))
    {
        ...
        if (dd_lock(&s, &pixels, &pitch))
        {
            ...
            {
                float dy = (float)(y - VIS_HEIGHT / 2);
                for (x = 0; x < VIS_WIDTH; x++)
                {
                    float dx = (float)(x - VIS_WIDTH / 2);
                    float d = (float)sqrt(dx * dx + dy * dy);
                    float a = (float)atan2(dy, dx);
                    int u = (int)((32.0f / (d + 1.0f) + t) * 64.0f) & 127;
                    int v = (int)((a / 3.14159f) * 64.0f + t * 12.0f) & 127;

                    pixels[y * pitch + x] = tex[v * 128 + u];
                }
            }
            dd_unlock(&s);
        }
    ...
    ok_(frame > 1, "rendered %d tunnel frames", frame - 1);
