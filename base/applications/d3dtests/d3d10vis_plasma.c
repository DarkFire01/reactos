 ...
 * PURPOSE:     Direct3D 10 visual: an animated plasma from a full-screen pixel shader
 */
...
/* Everything is done in the pixel shader; light.x is the animation time, which
   keeps struct d10_constants identical to the one the other scenes use. */
static const char hlsl_ps_plasma[] =
    "cbuffer constants : register(b0)\n"
    "{\n"
    "    float4x4 wvp;\n"
    "    float4x4 world;\n"
    ...
    "{\n"
    "    float t = light.x;\n"
    "    float x = i.uv.x * 13.0f;\n"
    "    float y = i.uv.y * 10.0f;\n"
    "    float v = sin(x + t) + sin(y - t) + sin((x + y) * 0.7f + t * 0.7f)\n"
    "            + sin(sqrt(x * x + y * y) * 0.9f - t * 1.3f);\n"
    "    float3 c;\n"
    "    c.r = 0.5f + 0.45f * sin(v * 1.1f);\n"
    "    c.g = 0.5f + 0.45f * sin(v * 1.1f + 2.0f);\n"
    "    c.b = 0.5f + 0.45f * sin(v * 1.1f + 4.0f);\n"
...
int main(int argc, char **argv)
{
    ...
    struct d10_vertex quad[4];
    struct d10_constants constants;
    DWORD sample[256];
    struct d10_scene s;
    struct vis_mat identity;
    float clear[4] = { 0.05f, 0.05f, 0.09f, 1.0f };
    ...
    test_begin("d3d10vis_plasma");

    hwnd = vis_create_window("Direct3D 10: pixel-shader plasma");
    if (!d10_open(&s, hwnd)) { skip_("no Direct3D 10 device"); goto done; }
    ok_(1, "created a Direct3D 10 device and swap chain");

    if (!(vsb = d10_compile(hlsl_vs, "vs_4_0")) || !(psb = d10_compile(hlsl_ps_plasma, "ps_4_0")))
    {
    ...
    vb = d10_make_buffer(s.device, quad, sizeof(quad), D3D10_BIND_VERTEX_BUFFER);
    ib = d10_make_buffer(s.device, quad_indices, sizeof(quad_indices), D3D10_BIND_INDEX_BUFFER);
    ...
    while (vis_frame(frame++))
    {
        ...
        d10_store_matrix(constants.wvp, &identity);
        d10_store_matrix(constants.world, &identity);
        constants.light[0] = t;
