 ...
 * PURPOSE:     Direct3D 7 visual: a directional light sweeping around a shaded model
...
#define LIT_CLEAR 0x00141024

int main(int argc, char **argv)
{
    struct d7_vertex verts[VIS_MODEL_VERTICES];
    DWORD sample[256];
    struct d7_scene s;
    ...
    test_begin("d3d7vis_lit");

    hwnd = vis_create_window("Direct3D 7: swept directional light");
    if (!d7_open(&s, hwnd))
    {
    ...
    if (SUCCEEDED(IDirect3DDevice7_GetCaps(s.device, &caps)))
        info_("the device supports %lu active light(s)",
              (unsigned long)caps.dwMaxActiveLights);

    /* The octahedron carries a real normal per face, which is what makes the
       shading move: each face catches the light differently as either the
       model or the light turns. */
    d7_fill_vertices(verts, vis_model, VIS_MODEL_VERTICES);
    d7_setup_camera(&s, 5.5f);

    memset(&material, 0, sizeof(material));
    material.dcvDiffuse.r = 0.85f;
    material.dcvDiffuse.g = 0.85f;
    material.dcvDiffuse.b = 0.9f;
    material.dcvDiffuse.a = 1.0f;
    material.dcvAmbient.r = 0.15f;
    material.dcvAmbient.g = 0.12f;
    material.dcvAmbient.b = 0.2f;
    material.dcvAmbient.a = 1.0f;
    hr = IDirect3DDevice7_SetMaterial(s.device, &material);
    ok_(SUCCEEDED(hr), "SetMaterial returned 0x%08lx", hr);

    memset(&light, 0, sizeof(light));
    light.dltType = D3DLIGHT_DIRECTIONAL;
    light.dcvDiffuse.r = 1.0f;
    light.dcvDiffuse.g = 0.95f;
    light.dcvDiffuse.b = 0.8f;
    light.dcvDiffuse.a = 1.0f;
    light.dvDirection.x = 0.8f;
    light.dvDirection.y = -0.5f;
    light.dvDirection.z = 0.0f;
    hr = IDirect3DDevice7_SetLight(s.device, 0, &light);
    ok_(SUCCEEDED(hr), "SetLight(0, directional) returned 0x%08lx", hr);
    hr = IDirect3DDevice7_LightEnable(s.device, 0, TRUE);
    ok_(SUCCEEDED(hr), "LightEnable(0, TRUE) returned 0x%08lx", hr);

    hr = IDirect3DDevice7_SetRenderState(s.device, D3DRENDERSTATE_LIGHTING, TRUE);
    ok_(SUCCEEDED(hr), "SetRenderState(LIGHTING, TRUE) returned 0x%08lx", hr);
    IDirect3DDevice7_SetRenderState(s.device, D3DRENDERSTATE_SPECULARENABLE, FALSE);
    IDirect3DDevice7_SetRenderState(s.device, D3DRENDERSTATE_AMBIENT, 0x20181828);
    ...
    /* The scene helper attaches a z-buffer, so the clear has to take it too. */
    ...
    while (vis_frame(frame++))
    {
        ...
        /* The light swings around the model while the model itself turns the
           other way, so no face keeps the same shade for long. It always keeps
           a downward component, so it never ends up straight overhead where
           every visible face would be lit the same. */
        light.dvDirection.x = (float)cos(t) * 0.85f;
        light.dvDirection.y = -0.5f;
        light.dvDirection.z = (float)sin(t) * 0.85f;
        IDirect3DDevice7_SetLight(s.device, 0, &light);
        ...
        IDirect3DDevice7_Clear(s.device, 0, NULL,
                D3DCLEAR_TARGET | (s.depth ? D3DCLEAR_ZBUFFER : 0), LIT_CLEAR, 1.0f, 0);

        if (SUCCEEDED(IDirect3DDevice7_BeginScene(s.device)))
        {
            hr = IDirect3DDevice7_DrawIndexedPrimitive(s.device, D3DPT_TRIANGLELIST,
                    D7_FVF, verts, VIS_MODEL_VERTICES,
                    (WORD *)vis_model_indices, VIS_MODEL_INDICES, 0);
            if (frame == 1)
    ...
    if (n)
    {
        vis_check_rendered(sample, n, LIT_CLEAR);
        /* One material, one light: any variation in the readback has to have
           come out of the lighting pipeline. */
        ok_(vis_count_distinct(sample, n, 0x00ffffff) > 2,
            "lighting shaded the faces into %d distinct colours",
    ...
    IDirect3DDevice7_LightEnable(s.device, 0, FALSE);
