 ...
 * PURPOSE:     Direct3D 11 visual: a translucent cube blended over a solid one
 */
...
/* The blended pass needs a per-draw opacity. light.w is spare in the constant
   buffer the other scenes use, so it carries the alpha and the layout of
   struct d11_constants stays exactly as the siblings have it. */
static const char hlsl_ps_alpha[] =
    "cbuffer constants : register(b0)\n"
    "{\n"
    "    float4x4 wvp;\n"
    "    float4x4 world;\n"
    ...
    "{\n"
    "    float3 n = normalize(i.nrm);\n"
    "    float  d = saturate(dot(n, normalize(float3(-0.4f, 0.7f, -0.6f))));\n"
    "    return float4(i.col.rgb * (0.35f + 0.65f * d), light.w);\n"
    "}\n";
...
int main(int argc, char **argv)
{
    ...
    ID3D11BlendState *opaque = NULL, *blended = NULL;
    ...
    DWORD sample[256];
    struct d11_scene s;
    struct vis_mat world, ghost, view, proj, wv, wvp, ry, rx, tr;
    float clear[4] = { 0.07f, 0.09f, 0.13f, 1.0f };
    ...
    test_begin("d3d11vis_alpha");

    hwnd = vis_create_window("Direct3D 11: alpha blending");
    ...
    if (!(vsb = d11_compile(hlsl_vs, "vs_4_0")) || !(psb = d11_compile(hlsl_ps_alpha, "ps_4_0")))
    {
    ...
    ID3D11Device_CreateBlendState(s.device, &bsd, &opaque);

    bsd.RenderTarget[0].BlendEnable = TRUE;
    bsd.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA;
    bsd.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA;
    ID3D11Device_CreateBlendState(s.device, &bsd, &blended);

    ok_(opaque != NULL && blended != NULL,
        "created an opaque and a SRC_ALPHA/INV_SRC_ALPHA blend state");
    if (!opaque || !blended)
    {
        skip_("this device would not give us the blend states");
        goto done;
    }
    ...
    while (vis_frame(frame++))
    {
        ...
        /* The solid cube first, with blending switched off. */
        ...
        constants.light[3] = 1.0f;
        d11_update_cb(s.context, cb, &constants, sizeof(constants));
        ID3D11DeviceContext_OMSetBlendState(s.context, opaque, factor, 0xffffffff);
        ID3D11DeviceContext_DrawIndexed(s.context, VIS_CUBE_INDICES, 0, 0);

        /* Then the same cube again, offset towards the camera and translucent,
           so the overlap shows the two colours mixed rather than one of them. */
        vis_translate(&tr, 1.15f, 0.35f, -0.6f);
        vis_mul(&ghost, &world, &tr);
        vis_mul(&wv, &ghost, &view);
        vis_mul(&wvp, &wv, &proj);
        memset(&constants, 0, sizeof(constants));
        d11_store_matrix(constants.wvp, &wvp);
        d11_store_matrix(constants.world, &ghost);
        constants.light[0] = -0.4f;
        constants.light[1] = 0.7f;
        constants.light[2] = -0.6f;
        constants.light[3] = 0.45f;
        d11_update_cb(s.context, cb, &constants, sizeof(constants));
        ID3D11DeviceContext_OMSetBlendState(s.context, blended, factor, 0xffffffff);
        ID3D11DeviceContext_DrawIndexed(s.context, VIS_CUBE_INDICES, 0, 0);
    ...
    if (n) vis_check_rendered(sample, n, 0xff211712);
    else skip_("could not read the back buffer");

    vis_wait_if_held();
done:
    D3DTEST_RELEASE(blended);
    D3DTEST_RELEASE(opaque);
    D3DTEST_RELEASE(cb);
    D3DTEST_RELEASE(ib);
    D3DTEST_RELEASE(vb);
