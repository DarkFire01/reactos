 ...
 * PURPOSE:     Direct3D 9 visual: interpenetrating cubes resolved by the depth buffer
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d9vis_depth");

    hwnd = vis_create_window("Direct3D 9: depth buffering");
    ...
    IDirect3DDevice9_SetRenderState(s.device, D3DRS_ZFUNC, D3DCMP_LESSEQUAL);
    IDirect3DDevice9_SetRenderState(s.device, D3DRS_CULLMODE, D3DCULL_CCW);
    IDirect3DDevice9_SetFVF(s.device, D9_CFVF);
    ok_(1, "enabled depth testing with LESSEQUAL");
    ...
    while (vis_frame(frame++))
    {
        ...
        {
            /* Three cubes on a circle that pass through each other: without a
               working z-buffer the draw order would show through. */
            for (i = 0; i < 3; i++)
            {
                float a = frame * 0.04f + i * 2.094f;

                vis_rotate_y(&ry, a * 1.5f);
                vis_translate(&tr, (float)cos(a) * 1.6f, 0.0f, (float)sin(a) * 1.6f);
                vis_mul(&world, &ry, &tr);
    ...
    ok_(frame > 1, "rendered %d frames of interpenetrating cubes", frame - 1);

    n = d9_sample(&s, sample, ARRAYSIZE(sample));
    if (n) vis_check_rendered(sample, n, 0xff181818);
