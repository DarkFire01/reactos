 ...
 * PURPOSE:     Direct3D 11 visual: a grid of cubes from one DrawIndexedInstanced call
 */
...
static D3DTEST_UNUSED int d11_sample(struct d11_scene *s, DWORD *out, int max)
{
    ...
    if (FAILED(ID3D11DeviceContext_Map(s->context, (ID3D11Resource *)staging, 0,
                                       D3D11_MAP_READ, 0, &m)))
        goto done;
    /* The usual (i*7, i*13) scatter is a fixed pattern, so a subject made of
       small pieces with gaps between them is missed the same way every run.
       Walk a contiguous run down the centre column instead: it is a column
       rather than a row because the camera sits above the origin looking down,
       so anything pushed away in +z climbs towards the horizon. */
    for (i = 0; i < max; i++)
    {
        int x = (int)td.Width / 2;
        int y = (int)td.Height / 2 - max / 2 + i;
...
/* One of these per instance: xyz is where the copy goes, w is how big it is. */
struct d11_instance { float offset[4]; float colour[4]; };

#define D11_INSTANCE_SIDE  5
#define D11_INSTANCES      (D11_INSTANCE_SIDE * D11_INSTANCE_SIDE)

static const D3D11_INPUT_ELEMENT_DESC d11_layout_instanced[] =
{
    { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,    0,  0, D3D11_INPUT_PER_VERTEX_DATA,   0 },
    { "NORMAL",   0, DXGI_FORMAT_R32G32B32_FLOAT,    0, 12, D3D11_INPUT_PER_VERTEX_DATA,   0 },
    { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,       0, 24, D3D11_INPUT_PER_VERTEX_DATA,   0 },
...
static const char hlsl_vs_instanced[] =
    "cbuffer constants : register(b0)\n"
    "{\n"
    "    float4x4 wvp;\n"
    "    float4x4 world;\n"
    ...
    "struct vs_in  { float3 pos : POSITION; float3 nrm : NORMAL;\n"
    "                float2 uv : TEXCOORD0; float4 col : COLOR;\n"
    "                float4 inst : TEXCOORD1; float4 icol : COLOR1; };\n"
    "struct vs_out { float4 pos : SV_POSITION; float3 nrm : NORMAL;\n"
    "                float2 uv : TEXCOORD0; float4 col : COLOR; };\n"
    "vs_out main(vs_in v)\n"
    "{\n"
    "    vs_out o;\n"
    "    float3 p = v.pos * v.inst.w + v.inst.xyz;\n"
    "    o.pos = mul(float4(p, 1.0f), wvp);\n"
    "    o.nrm = mul(float4(v.nrm, 0.0f), world).xyz;\n"
    "    o.uv  = v.uv;\n"
    "    o.col = v.icol;\n"
    "    return o;\n"
...
int main(int argc, char **argv)
{
    ...
    ID3D11Buffer *vb = NULL, *ib = NULL, *cb = NULL, *instvb = NULL;
    ID3D11Buffer *streams[2];
    ID3D10Blob *vsb = NULL, *psb = NULL;
    struct d11_vertex verts[VIS_CUBE_VERTICES];
    struct d11_instance instances[D11_INSTANCES];
    struct d11_constants constants;
    DWORD sample[256];
    struct d11_scene s;
    struct vis_mat world, view, proj, wv, wvp, ry, rx;
    float clear[4] = { 0.08f, 0.09f, 0.14f, 1.0f };
    UINT strides[2], offsets[2];
    int frame = 0, n, i, j;
    HWND hwnd;

    vis_parse_args(argc, argv);
    test_begin("d3d11vis_instanced");

    hwnd = vis_create_window("Direct3D 11: instanced cubes");
    ...
    if (!(vsb = d11_compile(hlsl_vs_instanced, "vs_4_0"))
        || !(psb = d11_compile(hlsl_ps_lit, "ps_4_0")))
    {
        skip_("could not compile the scene shaders");
        goto done;
    }
    ok_(1, "compiled the instancing vertex shader and the pixel shader");
    ...
    ID3D11Device_CreateInputLayout(s.device, d11_layout_instanced,
            ARRAYSIZE(d11_layout_instanced), ID3D10Blob_GetBufferPointer(vsb),
            ID3D10Blob_GetBufferSize(vsb), &layout);
    ok_(layout != NULL, "created an input layout with two per-instance elements");

    /* A flat grid in x and y. The copy at the centre sits on the axis the world
       rotation turns about, so it stays in the middle of the frame. */
    for (i = 0; i < D11_INSTANCE_SIDE; i++)
    {
        for (j = 0; j < D11_INSTANCE_SIDE; j++)
        {
            int k = i * D11_INSTANCE_SIDE + j;

            instances[k].offset[0] = (float)(j - D11_INSTANCE_SIDE / 2);
            instances[k].offset[1] = (float)(i - D11_INSTANCE_SIDE / 2);
            instances[k].offset[2] = 0.0f;
            instances[k].offset[3] = 0.28f;
            instances[k].colour[0] = 0.28f + 0.17f * (float)j;
            instances[k].colour[1] = 0.32f + 0.16f * (float)i;
            instances[k].colour[2] = 0.90f - 0.15f * (float)((i + j) % 5);
            instances[k].colour[3] = 1.0f;
        }
    }

    d11_fill(verts, vis_cube, VIS_CUBE_VERTICES);
    vb = d11_make_vb(s.device, verts, sizeof(verts));
    ib = d11_make_ib(s.device, vis_cube_indices, sizeof(vis_cube_indices));
    instvb = d11_make_vb(s.device, instances, sizeof(instances));
    cb = d11_make_cb(s.device, sizeof(constants));
    if (!vs || !ps || !layout || !vb || !ib || !instvb || !cb)
    {
    ...
    streams[0] = vb;
    streams[1] = instvb;
    strides[0] = sizeof(struct d11_vertex);
    strides[1] = sizeof(struct d11_instance);
    offsets[0] = 0;
    offsets[1] = 0;

    vis_perspective(&proj, 1.05f, (float)VIS_WIDTH / VIS_HEIGHT, 1.0f, 100.0f);
    vis_lookat(&view, 0.0f, 1.2f, -7.5f, 0.0f, 0.0f, 0.0f);
    ...
    while (vis_frame(frame++))
    {
        ...
        vis_rotate_y(&ry, t);
        vis_rotate_x(&rx, (float)sin(t * 0.7f) * 0.35f);
        vis_mul(&world, &rx, &ry);
        ...
        ID3D11DeviceContext_IASetVertexBuffers(s.context, 0, 2, streams, strides, offsets);
        ...
        ID3D11DeviceContext_DrawIndexedInstanced(s.context, VIS_CUBE_INDICES,
                D11_INSTANCES, 0, 0, 0);
        IDXGISwapChain_Present(s.swapchain, 0, 0);
    }
    ok_(frame > 1, "rendered %d frames of %d instances", frame - 1, D11_INSTANCES);

    n = d11_sample(&s, sample, ARRAYSIZE(sample));
    if (n) vis_check_rendered(sample, n, 0xff241714);
    else skip_("could not read the back buffer");

    vis_wait_if_held();
done:
    D3DTEST_RELEASE(cb);
    D3DTEST_RELEASE(instvb);
    D3DTEST_RELEASE(ib);
    D3DTEST_RELEASE(vb);
