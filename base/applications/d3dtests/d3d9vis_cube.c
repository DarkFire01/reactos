 ...
 * PURPOSE:     Direct3D 9 visual: a rotating gouraud-shaded cube
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d9vis_cube");

    hwnd = vis_create_window("Direct3D 9: rotating cube");
    if (!d9_open(&s, hwnd)) { skip_("no Direct3D 9 device"); goto done; }
    ok_(1, "created a Direct3D 9 device with a %dx%d back buffer", VIS_WIDTH, VIS_HEIGHT);
