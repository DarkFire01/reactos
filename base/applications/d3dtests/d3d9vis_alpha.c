 ...
 * PURPOSE:     Direct3D 9 visual: overlapping alpha-blended quads
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d9vis_alpha");

    hwnd = vis_create_window("Direct3D 9: alpha blending");
    ...
    IDirect3DDevice9_SetRenderState(s.device, D3DRS_ALPHABLENDENABLE, TRUE);
    IDirect3DDevice9_SetRenderState(s.device, D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
    IDirect3DDevice9_SetRenderState(s.device, D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
    ...
    IDirect3DDevice9_SetRenderState(s.device, D3DRS_ALPHABLENDENABLE, FALSE);
    vis_wait_if_held();
done:
    d9_close(&s);
