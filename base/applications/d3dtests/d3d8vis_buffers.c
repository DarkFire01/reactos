 ...
 * PURPOSE:     Direct3D 8 visual: a cube drawn from real vertex and index buffers
 */
...
int main(int argc, char **argv)
{
    ...
    test_begin("d3d8vis_buffers");

    hwnd = vis_create_window("Direct3D 8: cube from buffers");
    if (!d8_open(&s, hwnd)) { skip_("no Direct3D 8 device"); goto done; }

    d8_fill_colour(verts, vis_cube, VIS_CUBE_VERTICES);

    hr = IDirect3DDevice8_CreateVertexBuffer(s.device, sizeof(verts), D3DUSAGE_WRITEONLY,
            D8_CFVF, D3DPOOL_MANAGED, &vb);
    ok_(SUCCEEDED(hr) && vb != NULL, "CreateVertexBuffer returned 0x%08lx", hr);
    hr = IDirect3DDevice8_CreateIndexBuffer(s.device, sizeof(vis_cube_indices),
            D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_MANAGED, &ib);
    ok_(SUCCEEDED(hr) && ib != NULL, "CreateIndexBuffer returned 0x%08lx", hr);
    if (!vb || !ib) goto done;

    if (SUCCEEDED(IDirect3DVertexBuffer8_Lock(vb, 0, sizeof(verts), &data, 0)))
    {
        memcpy(data, verts, sizeof(verts));
        IDirect3DVertexBuffer8_Unlock(vb);
    }
    data = NULL;
    if (SUCCEEDED(IDirect3DIndexBuffer8_Lock(ib, 0, sizeof(vis_cube_indices), &data, 0)))
    {
    ...
    IDirect3DDevice8_SetStreamSource(s.device, 0, vb, sizeof(verts[0]));
    IDirect3DDevice8_SetIndices(s.device, ib, 0);
    ...
    while (vis_frame(frame++))
    {
        ...
        {
            IDirect3DDevice8_DrawIndexedPrimitive(s.device, D3DPT_TRIANGLELIST,
                    0, VIS_CUBE_VERTICES, 0, VIS_CUBE_INDICES / 3);
