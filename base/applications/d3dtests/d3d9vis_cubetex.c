 ...
 * PURPOSE:     Direct3D 9 visual: a textured rotating cube
 */
...
{
    IDirect3DTexture9 *texture = NULL;
    struct d9_vertex verts[VIS_CUBE_VERTICES];
    ...
    test_begin("d3d9vis_cubetex");

    hwnd = vis_create_window("Direct3D 9: textured cube");
    if (!d9_open(&s, hwnd)) { skip_("no Direct3D 9 device"); goto done; }

    vis_tex_rings(pixels, 64, 64);
    texture = d9_make_texture(s.device, pixels, 64);
    ok_(texture != NULL, "created a 64x64 ring texture");
    if (!texture) goto done;

    d9_fill(verts, vis_cube, VIS_CUBE_VERTICES);
    d9_camera(&s, 6.0f);
