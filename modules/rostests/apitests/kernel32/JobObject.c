...
test_SandboxPolicies(void)
{
    ...
    /* sandbox::Job::Init(JOB_LIMITED_USER) as the GPU delegate sets it up */
    hJob = CreateJobObjectW(NULL, NULL);
    ok(hJob != NULL, "CreateJobObject failed with %lu\n", GetLastError());
    if (hJob == NULL)
        return;
    ...
    /* JOB_LIMITED_USER restricts the UI... */
    SetLastError(0xDEADBEEF);
    Success = SetRestrictions(hJob, JOB_OBJECT_UILIMIT_WRITECLIPBOARD |
                                    JOB_OBJECT_UILIMIT_READCLIPBOARD |
                                    JOB_OBJECT_UILIMIT_HANDLES |
                                    JOB_OBJECT_UILIMIT_GLOBALATOMS);
    ...
    /* ...and then the GPU delegate takes all of it back off again */
    SetLastError(0xDEADBEEF);
    Success = SetRestrictions(hJob, 0);
    ok(Success == TRUE, "Excepting the restrictions away failed with %lu\n", GetLastError());
