 ...
 * --------
 *
 * The display driver hooks BitBlt and CopyBits.  For two common, profitable
 * cases we hand the operation off to the NV2A 2D engine via miniport IOCTLs:
 *
 *   1. Solid-colour fill: source surface is NULL, ROP is PATCOPY, the brush
 *      is a solid colour, and the destination is our framebuffer surface.
 *   2. Screen-to-screen copy: both surfaces are our framebuffer, ROP is
 *      SRCCOPY.
 *
 * Every other shape (patterned brushes, transparent blits, off-screen
 * surfaces, complex ROPs) falls through to EngBitBlt — the GDI engine then
 * draws into the bits we lent it via EngCreateBitmap, which on the Xbox is
 * the live framebuffer, so it just works (slowly).
 *
 * Clipping is honoured by walking the clip rectangle list when present.
 */
