 ...
 *
 * Splits cleanly into:
 *   - The IOCTL surface and payload structs (always defined).  Safe to include
 *     from user-mode GDI display drivers.
 *   - The in-kernel prototypes (only when XBOXVMP_KERNEL is defined or when
 *     xboxvmp.h has already been pulled in).
 */
...
/* Vendor-specific IOCTLs (0x800+ range reserved for IHV use). */
...
typedef struct _NV2A_DRAW_3D
{
    ...
    /* Destination rectangle in SCREEN pixels (the GL viewport mapped to the
     * desktop): where the caller's window/viewport lives.  The miniport renders
     * the triangle into this rectangle (NDC -> this rect) and presents ONLY this
     * rectangle, so it stays inside the GL window and doesn't disturb the rest of
     * the desktop.  DstW/DstH == 0 means "use the whole screen". */
    unsigned long  DstX;
    unsigned long  DstY;
    unsigned long  DstW;
    unsigned long  DstH;
