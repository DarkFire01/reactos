 ...
 *   - Framebuffer set-up, reusing whatever resolution the XBE firmware (or
 *     Cromwell) configured before handoff.
 *   - 8 bpp palette / colour-register support via the NV2A RAMDAC.
 *   - Power management mapped onto NV_PMC_ENABLE.
 *   - A 2D accelerator using NV2A's NV_IMAGE_BLIT / NV04_GDI_RECTANGLE_TEXT
 *     engines, exposed to user-mode through three private IOCTLs.  See
 *     nv2a_accel.c for the hardware programming detail.
 */
