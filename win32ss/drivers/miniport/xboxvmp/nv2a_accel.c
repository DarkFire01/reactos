 ...
 *
 * Hardware programming sequences are derived from:
 *   - nouveau / envytools (the open-source NVIDIA reverse-engineering project)
 *   - nxdk pbkit (MIT-licensed Xbox homebrew GPU library)
 *   - Cromwell's video bring-up
 *
 * Channel layout
 * --------------
 *
 ...
 * -------
 *
 * This code re-programs PMC / PFIFO / PGRAPH, which means it disturbs whatever
 * the firmware (or Cromwell) had previously set up.  We *preserve* the CRTC
 * framebuffer-start register across the reset so that the display stays live.
 *
 * The accelerator is gated by ``DeviceExtension->AccelEnabled``; on a build
 * where ``XBOX_NV2A_ACCEL_DEFAULT_ON`` is left undefined, init records the
 * channel topology but leaves the engine disabled, and the IOCTL layer falls
 * back to CPU memset / memcpy.  Flip the macro to 1 to enable hardware
...
/* ntoskrnl exports the videoport miniport headers don't declare.  We import
 * ntoskrnl (see CMakeLists add_importlibs), so these resolve at link time.
 * Used to translate the contiguous texture-pool virtual address to the
 * guest-physical address the NV2A DMAs from (UMA: CPU phys == GPU offset). */
PHYSICAL_ADDRESS NTAPI MmGetPhysicalAddress(PVOID BaseAddress);
VOID NTAPI MmFreeContiguousMemory(PVOID BaseAddress);

#ifndef XBOX_NV2A_ACCEL_DEFAULT_ON
#define XBOX_NV2A_ACCEL_DEFAULT_ON 0
#endif
...
#else
    INFO_(IHVVIDEO,
          "NV2A accel: scaffolded but disabled (rebuild with "
          "XBOX_NV2A_ACCEL_DEFAULT_ON=1 or set the registry switch to enable)\n");
#endif
...
/*
 * The NV2A has no usable fixed-function pipeline, so even one triangle needs a
 * vertex program and configured register combiners.  Both the passthrough VP
 * (oPos = v0, oD0 = v3) and the "output = diffuse" combiner words below are the
 * verbatim output of nxdk's own vp20/fp20 compilers — known-good in xemu.
 *
 * The caller submits CLIP-space vertices; the passthrough VP forwards them and
 * the hardware does the perspective divide + viewport + Gouraud interpolation.
 */
 ...
 * space here is exactly what's required.
 *
 * Earlier we used a hand-crafted 2-instruction passthrough (MOV oPos,v0 / MOV
 * oCol0,v3); it rendered nothing in xemu.  This 8-instruction program is the one
 * nxdk ships and that xemu's NV2A renderer is known to rasterise. */
...
/* ----- PGRAPH 3D enable + channel-0 context validity (xemu-verified) ------ *
 * xemu does NO graphics/FIFO context save-restore on a switch; it only checks
 * CTX_CONTROL.CHID_VALID && CTX_USER.CHID==channel && PGRAPH FIFO access.  So
 * we enable the 3D pipeline debug bits and pre-establish channel 0 as valid
 * BEFORE binding Kelvin, so its SET_OBJECT switch passes on the first try with
 * no stall and no FIFO poking (which would desync and crash the pusher). */
static VOID
Nv3dPgraphSetup(PXBOXVMP_DEVICE_EXTENSION Dx)
{
    /* FULL PGRAPH initialisation, captured verbatim from a RETAIL Xbox BIOS boot
     * (the exact PGRAPH register state nxdk/pbkit rely on).  Our previous minimal
     * setup left the raster/setup blocks (0x400780+, 0x400900-0x4009a8) and many
     * config regs UNwritten, so xemu's 3D draws produced ZERO fragments while
     * CLEAR_SURFACE worked.  nxdk's own triangle renders under this state; ours
     * did not under the minimal one — this replicates it.  We use channel 0 just
     * like the BIOS, so the context/CHID values apply directly. */
...
Nv2a3dInitialize(PXBOXVMP_DEVICE_EXTENSION Dx)
{
     ...
     * CRITICAL: this MUST NOT be reloaded per-draw — loading transform constants
     * mid-stream while the program is active stalls PGRAPH under xemu, so the
     * per-draw WaitIdle spins to its timeout and the guest FREEZES.  The matrix is
     * constant (depends only on screen size), so loading it once is sufficient.
     * Maps NDC [-1,1] -> screen [0,W]x[H,0] (Y flipped, framebuffer top-down); the
     * VP computes oPos = v0.x*c96 + v0.y*c97 + v0.z*c98 + c99 = screen coords, and
     * xemu's program-mode inverse-viewport recovers clip space.  (nxdk matrix_viewport.) */
    PbPush(Dx, NV2A_METHOD(SUBCH_3D, NV097_SET_TRANSFORM_CONSTANT_LOAD, 1));
    PbPush(Dx, 96);
    PbPush(Dx, NV2A_METHOD(SUBCH_3D, NV097_SET_TRANSFORM_CONSTANT, 16));
...
Nv2a3dDrawTriangles(PXBOXVMP_DEVICE_EXTENSION Dx, const NV2A_DRAW_3D *Draw)
{
    ...
    if (doDraw)
    {
        ...
        if (TexEnable && TexW && TexH)
        {
             ...
             * (DECAL/BLEND words derived from the combiner encoding — verify in xemu.) */
            {
