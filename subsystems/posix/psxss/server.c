 ...
 * PURPOSE:     API port worker thread dispatch loop
...
/**
 * @brief API port worker loop. Dispatches each request through g_OpHandlers; the
 * handler fills the reply in place and it is sent on the next receive.
 */
VOID
NTAPI
PsxApiServerLoop(
    _In_opt_ PVOID Parameter)
{
    ...
    for (;;)
    {
        ...
        {
            PSXTRACE("ApiServerLoop: NtReplyWaitReceivePort status 0x%08lx\n", Status);
            Reply = NULL;
            continue;
        }
        ...
        switch (Request.Header.u2.s2.Type & 0x000000FF)
        {
            ...
            case LPC_REQUEST:
                ...
                /* Skip tracing read, write and poll since they are very frequent */
                if (ApiNumber != PsxApiRead &&
                    ApiNumber != PsxApiWrite &&
                ...
                /* Extension opcodes sit above PsxApiMaxApiNumber and are handled first */
                ...
                {
                    PSXTRACE("ApiServerLoop: api 0x%lx not implemented, returning ENOSYS\n", ApiNumber);
