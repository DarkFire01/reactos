 ...
 *
 * The freeze signal here is the maskable IPI the reference uses, delivered
 * through KiIpiSend()/KiIpiServiceRoutine() as IPI_FREEZE.
 */
/*
    ...
    VOID)
{
                                            ...
                                            NULL) != NULL)
    {
        ...
        {
            /*
             * Answer a freeze aimed at us right here.
             *
             * KeFreezeExecution() disabled interrupts before calling us, so the
             * freeze IPI the owner has just sent cannot be delivered and
             * KiIpiServiceRoutine() will never run for it.  The owner would
             * wait for a FROZEN we are unable to report while we wait for it to
             * finish, and both processors would spin until the machine was
             * reset - which is what two processors entering the debugger at
             * once produced.  The reference has the same problem and the same
             * answer: KeFreezeExecution calls KiIpiProcessRequests every time
             * it fails to take KiFreezeExecutionLock.  This is that call,
             * reduced to the one request that can be outstanding here.
             *
             * x64 does not need it, because it freezes with an NMI.
             *
             * Nothing saves our processor state here, so the debugger cannot
             * show this processor's context.  It can still inspect the owner
             * and continue the system, which is the part that matters.
             */
                               ...
                               ~IPI_FREEZE) & IPI_FREEZE)
            {
                ...
                /* Released by anything that is not FROZEN, as above */
                while ((CurrentPrcb->IpiFrozen & IPI_FROZEN_STATE_MASK) ==
                       IPI_FROZEN_STATE_FROZEN)
                {
                    YieldProcessor();
