    ...
    _In_ PDEVICE_NODE DeviceNode)
{
    ...
    for (Link = ArbiterEntry->ResourceList.Flink; Link != &ArbiterEntry->ResourceList; Link = Link->Flink)
    {
    ...
    _Out_ PCM_RESOURCE_LIST *TranslatedList)
{
    ...
    *TranslatedList = NULL;
    ...
    {
        PIOP_REQUIREMENT Requirement = &Configuration->Requirements[Index];
        PCM_PARTIAL_RESOURCE_DESCRIPTOR Raw = &Lists[0]->List[0].PartialResourceList.PartialDescriptors[Index];
        PCM_PARTIAL_RESOURCE_DESCRIPTOR Translated = &Lists[1]->List[0].PartialResourceList.PartialDescriptors[Index];
    ...
    _Out_ PCM_RESOURCE_LIST *TranslatedList)
{
    ...
    *TranslatedList = NULL;
    ...
    Configurations = ExAllocatePoolZero(PagedPool,
                                        RequirementsList->AlternativeLists * sizeof(IOP_CONFIGURATION),
                                        TAG_IO_ARBITER);
    ...
    _In_ PDEVICE_NODE DeviceNode)
{
    ...
    ResourceList = (DeviceNode->ResourceList != NULL) ? DeviceNode->ResourceList : DeviceNode->BootResources;
    ...
    _In_ PDEVICE_NODE DeviceNode)
{
    ...
    /* Free the ranges the device already holds, so it does not conflict with itself */
    ...
    /* Reserve the boot configuration again, it was freed above */
    if (DeviceNode->BootResources != NULL)
        IopArbiterReserveBootConfig(DeviceNode);
    ...
    _In_opt_ PDEVICE_NODE OwningNode)
{
    ...
    Requirements = ExAllocatePoolZero(PagedPool,
                                      HeaderSize + PartialList->Count * sizeof(IO_RESOURCE_DESCRIPTOR),
                                      TAG_IO_ARBITER);
    if (Requirements == NULL)
        return NULL;
    ...
    _Out_opt_ PCM_PARTIAL_RESOURCE_DESCRIPTOR ConflictingDescriptor)
{
    ...
    for (Index = 0; Index < PartialList->Count; Index++)
    {
        ...
        if (DeviceNode != NULL && NT_SUCCESS(IopFindRequirementHandlers(&Requirement, InterfaceType)))
        {
        ...
        Parameters.Parameters.QueryConflict.ConflictingResource = &Requirement.Top->Entry.Alternatives[0];
