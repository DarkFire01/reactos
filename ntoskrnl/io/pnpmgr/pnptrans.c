...
typedef struct _IOP_FAILED_TRANSLATION
{
    PDEVICE_NODE DeviceNode;
    CM_PARTIAL_RESOURCE_DESCRIPTOR Resource;
} IOP_FAILED_TRANSLATION;

IOP_FAILED_TRANSLATION IopFailedTranslations[32];
ULONG IopFailedTranslationCount;
#endif
    ...
    _In_ INTERFACE_TYPE InterfaceType)
{
    ...
    if ((ULONG)InterfaceType > (ULONG)ACPIBus || InterfaceType == PNPBus)
        return NULL;
    ...
    _In_ PDEVICE_NODE DeviceNode)
{
    ...
    if (Next != NULL)
        InsertTailList(Next, &DeviceNode->LegacyBusListEntry);
    ...
    for (Index = 0; Index < AlternativeCount; Index++)
    {
        if (Results[Index].Count != 0)
        {
            RtlCopyMemory(Cursor,
                          Results[Index].Descriptors,
                          Results[Index].Count * sizeof(*Cursor));
...
static
VOID
IopSaveFailedTranslation(
    _In_ PDEVICE_NODE DeviceNode,
    _In_ PCM_PARTIAL_RESOURCE_DESCRIPTOR Resource)
{
    if (IopFailedTranslationCount < RTL_NUMBER_OF(IopFailedTranslations))
    {
        IopFailedTranslations[IopFailedTranslationCount].DeviceNode = DeviceNode;
        IopFailedTranslations[IopFailedTranslationCount].Resource = *Resource;
        IopFailedTranslationCount++;
    }
}
#else
#define IopSaveFailedTranslation(DeviceNode, Resource)
#endif
    ...
    _Out_ PCM_PARTIAL_RESOURCE_DESCRIPTOR Translated)
{
    ...
    while (Node != NULL)
    {
        ...
        if (Node == IopRootDeviceNode && !DidVisitLegacyBus)
        {
        ...
        if (IopGetCachedTranslator(Node, Raw->Type, &Translator) && Translator != NULL)
        {
            ...
            if (!NT_SUCCESS(Status))
            {
                if ((DeviceNode != NULL) && (Status != STATUS_RETRY))
                {
                    ...
                    IopSaveFailedTranslation(DeviceNode, &Current);
                }
    ...
    _Out_ PPHYSICAL_ADDRESS TranslatedAddress)
{
         ...
         Node = IopGetResourceParent(Node))
    {
        ...
        IsCached = IopGetCachedTranslator(Node, Current.Type, &Translator);
        if (!IsCached)
        {
        ...
        if (!IsCached && Queried.InterfaceDereference != NULL)
            Queried.InterfaceDereference(Queried.Context);
