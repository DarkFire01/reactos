    ...
    _Out_ PINTERRUPT_CONNECTION_DATA *ConnectionData)
{
    ...
    *ConnectionData = NULL;
    ...
    Status = IoGetDevicePropertyData(Pdo, &IopInterruptConnectionDataKey, 0, 0, 0, NULL, &Size, &Type);
    ...
    Status = IoGetDevicePropertyData(Pdo, &IopInterruptConnectionDataKey, 0, 0, Size, Data, &Size, &Type);
    ...
    _In_ BOOLEAN FloatingSave)
{
    ...
    *MessageTable = NULL;
    ...
    Messages->Links = (PIOP_MESSAGE_INTERRUPT_LINK)((PUCHAR)Messages + IOP_MESSAGE_TABLE_SIZE(MessageCount));
    ...
    _In_ USHORT Group)
{
    ...
    for (Index = 0; Index < VectorCount; Index++)
    {
        ...
        if (Match->Type == InterruptTypeControllerInput && Match->ControllerInput.WakeInterrupt != 0)
    ...
    {
        DPRINT1("IoConnectInterruptEx: PDO %p has no vector %lu at irql %u mode %u group %u affinity 0x%p\n",
    ...
    _In_ PIO_DISCONNECT_INTERRUPT_PARAMETERS Parameters)
{
    ...
    switch (Parameters->Version)
    {
        ...
        case CONNECT_LINE_BASED:
            IopDisconnectLineInterrupts(CONTAINING_RECORD(Parameters->ConnectionContext.InterruptObject,
                                                          IOP_LINE_INTERRUPTS,
                                                          Interrupt));
            break;
