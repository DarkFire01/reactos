...
static FORCEINLINE ULONG _APICRead(ULONG Offset)
{
    PULONG p;
...
#else
static FORCEINLINE VOID APICWrite(ULONG Offset,
                               ULONG Value)
{
    PULONG p;
...
#else
static FORCEINLINE ULONG APICRead(ULONG Offset)
{
    PULONG p;
    ULONG CPU = (_APICRead(APIC_ID) & APIC_ID_MASK) >> 24;
...
static FORCEINLINE ULONG ThisCPU(VOID)
{
    return (APICRead(APIC_ID) & APIC_ID_MASK) >> 24;
}

static FORCEINLINE VOID APICSendEOI(VOID)
{
    // Send the EOI
