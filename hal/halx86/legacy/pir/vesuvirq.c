...
/*
 * The route registers sit behind the 0x24/0x26 index/data pair. Links 1-4
 * are nibbles of indexes 0x10 and 0x11, and bits 0-3 of index 0x12 are
 * cleared for links whose IRQ is level triggered.
 */
    ...
    _In_ UCHAR Irq)
{
    ...
    HalpVesuviusWrite(Index, HalpPirSetNibble(HalpVesuviusRead(Index), HalpPirPairHighNibble(Link), Irq));
    return STATUS_SUCCESS;
}
...
{
    UCHAR Edge = HalpVesuviusRead(VESUVIUS_EDGE_INDEX) & 0x0F;
    UCHAR Link, Irq;
