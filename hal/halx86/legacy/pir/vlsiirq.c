...
/*
 * Register 0x74 holds a 3-bit IRQ code for each of the links 1 to 4, and bits
 * 16 to 23 enable the eight codes. The low byte of 0x5C marks the codes that
 * are level triggered, and is mirrored into 0x78.
 */
    ...
    _Out_ PUCHAR Irq)
{
    ...
    Code = (UCHAR)((Route >> ((Link - 1) * VLSI_CODE_BITS)) & VLSI_CODE_MASK);
    ...
    _In_ UCHAR Irq)
{
    ...
    {
        Codes[Index] = (UCHAR)((Route >> (Index * VLSI_CODE_BITS)) & VLSI_CODE_MASK);
 ...
 * and its trigger bit in the word register 0x88.
 */
    ...
    _In_ UCHAR Irq)
{
    ...
    Previous = (UCHAR)((Route >> Shift) & 0x0F);
