...
/*
 * Links 1 to 4 each have a 3-bit field in register 0x40 for IRQs 5, 9 to 12,
 * 14 and 15, and a 2-bit field in register 0x50 for IRQs 3, 4 and 7. Only one
 * of the two is nonzero. Bits 17 to 23 of 0x40 and bits 8 to 10 of 0x50 mark
 * the codes that are level triggered.
 */
    ...
    _Out_ PUCHAR Irq)
{
    ...
    Code = (UCHAR)((HalpPirReadRouterLong(VIPER_WIDE_REGISTER) >> Shift) & 0x07);
    *Irq = HalpViperCodeToIrq(HalpViperWideCodes, Code);

    if (*Irq == 0)
    {
        Shift = (Link - 1) * VIPER_NARROW_BITS;
        Code = (UCHAR)((HalpPirReadRouterLong(VIPER_NARROW_REGISTER) >> Shift) & 0x03);
