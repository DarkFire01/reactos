    ...
    _In_ PHALP_PCI_LINK Link)
{
    ...
    if (Irq != 0)
    {
        ...
        DPRINT("HalIRQ: %p link 0x%x shares IRQ %lu\n", DeviceObject, Link->Link, Irq);
        return Irq;
    }
    ...
    for (Irq = 1; Irq <= HALP_PIC_LAST_IRQ; Irq++)
    {
        ...
        if (Irq == Firmware)
        {
            DPRINT("HalIRQ: %p link 0x%x keeps firmware IRQ %lu\n", DeviceObject, Link->Link, Irq);
            return Irq;
        }
    ...
    DPRINT("HalIRQ: %p link 0x%x mask 0x%x firmware IRQ %u unavailable, picked %lu\n",
    ...
    _In_ PARBITER_INSTANCE Arbiter)
{
    ...
    for (Link = HalpLegacyPCFirstLink(); Link; Link = Link->Next)
    {
        ...
        DPRINT("HalIRQ: link 0x%x IRQ %u -> %u\n", Link->Link, Current, Wanted);
