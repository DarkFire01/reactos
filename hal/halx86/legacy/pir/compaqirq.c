...
/* OSB and CMC-2 route registers are accessed through an index/data port pair */
#define CPQ_INDEX_PORT          0xC00
#define CPQ_DATA_PORT           0xC01

/* OSB links 1-4 are indexes 4-7, with the IRQ in the low nibble */
#define CPQ_OSB_LINKS           4
#define CPQ_OSB_FIRST_INDEX     4

/* CMC-2 links 1-6, with the IRQ in the high nibble */
#define CPQ_CMC2_LINKS          6

static const UCHAR HalpCompaqCmc2Indexes[CPQ_CMC2_LINKS] = { 0x00, 0x01, 0x06, 0x07, 0x04, 0x05 };

/* MISC-3 links select an entry of the 0xAE/0xAF index/data registers on one of two functions */
    ...
    _Out_ PUCHAR Irq)
{
    ...
    *Irq = HalpPirGetNibble(HalpCompaqIndexedRead(CPQ_OSB_FIRST_INDEX + Link - 1), FALSE);
    ...
    _In_ UCHAR Irq)
{
    ...
    HalpCompaqIndexedUpdate(CPQ_OSB_FIRST_INDEX + Link - 1, FALSE, Irq);
    return STATUS_SUCCESS;
}
...
/* Links 8 and 9 are accepted in the table but cannot be steered */
static
BOOLEAN
NTAPI
...
/* Bridge entries are on bus 0, device 15, function 0 */
static
UCHAR
HalpCompaqMisc3Read(
...
/* Points the index register at the link's entry */
static
BOOLEAN
    ...
    _Out_ PBOOLEAN OnBridge)
{
    ...
    {
        *OnBridge = FALSE;
        HalpCompaqMisc3Write(FALSE, CPQ_MISC3_INDEX, Link - CPQ_MISC3_ROUTER_FIRST);
        return TRUE;
    }
    ...
    {
        *OnBridge = TRUE;
        HalpCompaqMisc3Write(TRUE, CPQ_MISC3_INDEX, Link - CPQ_MISC3_BRIDGE_FIRST);
        return TRUE;
    }
    ...
    _In_ UCHAR Irq)
{
    ...
    HalpCompaqMisc3Write(OnBridge, CPQ_MISC3_DATA, (Irq != 0) ? (UCHAR)(Irq << 4) : CPQ_MISC3_DISABLED);
    return STATUS_SUCCESS;
}
