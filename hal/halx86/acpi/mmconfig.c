...
HalpPublishMmConfigRanges(VOID)
{
    UNICODE_STRING KeyName =
        RTL_CONSTANT_STRING(L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\Arbiters\\ReservedResources");
    ...
    EntryCount = (Mcfg->Header.Length - FIELD_OFFSET(MCFG_TABLE, Allocations)) / sizeof(MCFG_ALLOCATION);

    Windows = ExAllocatePoolWithTag(PagedPool, EntryCount * sizeof(HALP_PCI_SEGMENT_WINDOW), TAG_HAL);
    if (Windows == NULL)
    ...
    List->List[0].Count = HalpDescribeSegmentWindows(Windows, WindowCount, List->List[0].Descriptors);
