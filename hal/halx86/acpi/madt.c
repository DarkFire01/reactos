    ...
    _In_ ULONG Flags)
{
    ...
    /* An online capable processor is not present yet, it can only be added at runtime */
    if (!(Flags & ACPI_MADT_ENABLED))
    {
        return;
    }

    /* LapicId holds 8 bits, and 0xFF is the broadcast ID */
    if (ApicId >= 0xFF)
    {
    ...
    if (Index == RTL_NUMBER_OF(HalpStaticProcessorIdentity))
    {
        HalpMadtIgnoredEntries++;
        return;
    }

    HalpProcessorIdentity[Index].ProcessorId = (UCHAR)ProcessorId;
    HalpProcessorIdentity[Index].LapicId = (UCHAR)ApicId;
...
{
    /* Keep the first unit when an ID is duplicated */
    ...
    _In_ ACPI_TABLE_MADT *MadtTable)
{
    ...
    {
        /* The next entry is only found through the length, so a bad one ends the walk */
...
HalpMadtPlaceBootProcessor(VOID)
{
    ...
    else
    {
        ...
        RtlMoveMemory(&HalpProcessorIdentity[1],
                      ...
                      Index * sizeof(PROCESSOR_IDENTITY));
    }
    ...
    _In_ PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    // TODO: We support only legacy APIC for now, what about ACPI_MADT_PCAT_COMPAT?
