 ...
 *
 * @param[in] Vector
 * The vector to request.
 *
 * @remarks
 * A fixed command has to assert. Some implementations drop a de-asserting one,
 * which would leave the IRR wait below spinning.
 */
FORCEINLINE
VOID
...
/**
 * @brief
 * Points an I/O APIC input at a vector and unmasks it.
 *
 * @param[in] Input
 ...
 * Processors the input should reach.
 */
static
NTSTATUS
NTAPI
 ...
 * stays allocated until its owner gives it back.
 */
NTSTATUS
NTAPI
HalDisableInterrupt(
