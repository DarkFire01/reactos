 ...
 * The maximum number of physical processor packages allowed.
 *
    ...
    _In_ ULONG ProcessorId)
{
    ...
    /* Processors in one package share the APIC ID bits above this mask */
    PackageMask = 1;
    while (PackageMask < KeGetCurrentPrcb()->LogicalProcessorsPerPhysicalProcessor)
    {
        PackageMask <<= 1;
    }
    PackageMask = ~(PackageMask - 1);

    /* Collect the packages of the running processors */
    PackageCount = 0;
    SamePackage = FALSE;
    for (Processor = 0; Processor < (ULONG)KeNumberProcessors; Processor++)
    {
        if (!NT_SUCCESS(HalGetProcessorIdByNtNumber(Processor, &ApicId)))
        {
            continue;
        }

        PackageId = ApicId & PackageMask;
        if (PackageId == (ProcessorId & PackageMask))
        {
            SamePackage = TRUE;
        }
        ...
        {
            Packages[PackageCount++] = PackageId;
        }
    }

    if (PackageCount > ProcessorNumber)
    {
        return STATUS_UNSUCCESSFUL;
    }

    /* At the limit, the processor must belong to a running package */
    if ((PackageCount == ProcessorNumber) && !SamePackage)
    {
        return STATUS_UNSUCCESSFUL;
    }
    ...
    _In_ ULONG Attributes)
{
    ...
    /* An append write needs data */
    if ((Attributes & EFI_VARIABLE_APPEND_WRITE) && (ValueLength == 0))
    {
        return STATUS_INVALID_PARAMETER;
    }
