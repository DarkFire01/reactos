...
/* KeRemoveEntryDeviceQueue asserts on a queue that drained before the cancel got here */
static
BOOLEAN
NTAPI
HalpRemoveChannelWaiter(
    _In_ PKDEVICE_QUEUE Queue,
    _In_ PKDEVICE_QUEUE_ENTRY Entry)
{
    KLOCK_QUEUE_HANDLE LockHandle;
    BOOLEAN Removed;

    KeAcquireInStackQueuedSpinLockAtDpcLevel(&Queue->Lock, &LockHandle);
    ...
    _In_ PVOID DmaTransferContext)
{
    ...
    if (!HalpRemoveChannelWaiter(&AdapterObject->ChannelWaitQueue,
    ...
    _In_ BOOLEAN Synchronous)
{
    ...
    if (Synchronous)
    {
        KeAcquireInStackQueuedSpinLockAtDpcLevel(&AdapterObject->ChannelWaitQueue.Lock, &LockHandle);
