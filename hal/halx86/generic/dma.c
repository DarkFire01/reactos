...
static NTSTATUS NTAPI HalGetDmaAdapterInfo(PADAPTER_OBJECT, PDMA_ADAPTER_INFO);
static NTSTATUS NTAPI HalGetDmaTransferInfo(PADAPTER_OBJECT, PMDL, ULONGLONG, ULONG, BOOLEAN, PDMA_TRANSFER_INFO);
static NTSTATUS NTAPI HalInitializeDmaTransferContext(PADAPTER_OBJECT, PVOID);
static PVOID NTAPI HalAllocateCommonBufferEx(PADAPTER_OBJECT, PPHYSICAL_ADDRESS, ULONG, PPHYSICAL_ADDRESS, BOOLEAN, NODE_REQUIREMENT);
static NTSTATUS NTAPI HalAllocateAdapterChannelEx(PADAPTER_OBJECT, PDEVICE_OBJECT, PVOID, ULONG, ULONG, PDRIVER_CONTROL, PVOID, PVOID *);
static NTSTATUS NTAPI HalConfigureAdapterChannel(PADAPTER_OBJECT, ULONG, PVOID);
static BOOLEAN NTAPI HalCancelAdapterChannel(PADAPTER_OBJECT, PDEVICE_OBJECT, PVOID);
static NTSTATUS NTAPI HalMapTransferEx(PADAPTER_OBJECT, PMDL, PVOID, ULONGLONG, ULONG, PULONG, BOOLEAN, PSCATTER_GATHER_LIST, ULONG, PDMA_COMPLETION_ROUTINE, PVOID);
static NTSTATUS NTAPI HalGetScatterGatherListEx(PADAPTER_OBJECT, PDEVICE_OBJECT, PVOID, PMDL, ULONGLONG, ULONG, ULONG, PDRIVER_LIST_CONTROL, PVOID, BOOLEAN, PDMA_COMPLETION_ROUTINE, PVOID, PSCATTER_GATHER_LIST *);
static NTSTATUS NTAPI HalBuildScatterGatherListEx(PADAPTER_OBJECT, PDEVICE_OBJECT, PVOID, PMDL, ULONGLONG, ULONG, ULONG, PDRIVER_LIST_CONTROL, PVOID, BOOLEAN, PVOID, ULONG, PDMA_COMPLETION_ROUTINE, PVOID, PSCATTER_GATHER_LIST *);
static NTSTATUS NTAPI HalFlushAdapterBuffersEx(PADAPTER_OBJECT, PMDL, PVOID, ULONGLONG, ULONG, BOOLEAN);
static VOID NTAPI HalFreeAdapterObject(PADAPTER_OBJECT, IO_ALLOCATION_ACTION);
static NTSTATUS NTAPI HalCancelMappedTransfer(PADAPTER_OBJECT, PVOID);
...
HalpInitDma(VOID)
{
    ...
    HalpDmaOperationsV3.InitializeDmaTransferContext = (PINITIALIZE_DMA_TRANSFER_CONTEXT)HalInitializeDmaTransferContext;
    HalpDmaOperationsV3.AllocateCommonBufferEx = (PALLOCATE_COMMON_BUFFER_EX)HalAllocateCommonBufferEx;
    HalpDmaOperationsV3.AllocateAdapterChannelEx = (PALLOCATE_ADAPTER_CHANNEL_EX)HalAllocateAdapterChannelEx;
    HalpDmaOperationsV3.ConfigureAdapterChannel = (PCONFIGURE_ADAPTER_CHANNEL)HalConfigureAdapterChannel;
    ...
    HalpDmaOperationsV3.GetScatterGatherListEx = (PGET_SCATTER_GATHER_LIST_EX)HalGetScatterGatherListEx;
    HalpDmaOperationsV3.BuildScatterGatherListEx = (PBUILD_SCATTER_GATHER_LIST_EX)HalBuildScatterGatherListEx;
    ...
    _Out_opt_ PSCATTER_GATHER_LIST *ScatterGatherList)
{
    ...
    if (NT_SUCCESS(Status) && !ExecutionRoutine && (List->Reserved != (ULONG_PTR)ScatterGatherContext))
    ...
    _In_ PVOID Context)
{
    ...
    return Transfer->ExecutionRoutine(DeviceObject, Irp, MapRegisterBase, Transfer->ExecutionContext);
}
    ...
    _Inout_ PDMA_ADAPTER_INFO AdapterInfo)
{
    ...
    if (AdapterObject->MasterDevice)
    {
        ...
        AdapterInfo->V1.Flags = AdapterObject->NeedsMapRegisters ? 0 : ADAPTER_INFO_SYNCHRONOUS_CALLBACK;
    }
    else
    {
    ...
    _Inout_ PDMA_TRANSFER_INFO TransferInfo)
{
    ...
    Status = HalpGetTransferCounts(AdapterObject, Mdl, Offset, Length, &MapRegisterCount, &ElementCount);
    if (!NT_SUCCESS(Status))
        return Status;
    ...
    _In_ PVOID DmaTransferContext)
{
    ...
    if (!KeRemoveEntryDeviceQueue(&AdapterObject->ChannelWaitQueue, &Transfer->Wcb.WaitQueueEntry) &&
        (MasterAdapter != NULL))
    {
    ...
    _In_opt_ PVOID CompletionContext)
{
    ...
    if ((ScatterGatherBuffer == NULL) ||
        (ScatterGatherBufferLength < FIELD_OFFSET(SCATTER_GATHER_LIST, Elements) + sizeof(SCATTER_GATHER_ELEMENT)))
    {
        return STATUS_INVALID_PARAMETER;
    }
